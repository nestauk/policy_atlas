...
def test_valid_plan_compiles() -> None:
    plan = Plan(component="echo", source_ref="syn-001")
    config = compile(plan)
    assert isinstance(config, Config)
    assert config.component == "echo"
    assert config.source_ref == "syn-001"


def test_invalid_component_raises_at_plan_construction() -> None:
    with pytest.raises(ValidationError):
        Plan(component="nonexistent", source_ref="syn-001")


def test_invalid_source_ref_raises_at_plan_construction() -> None:
    with pytest.raises(ValidationError):
        Plan(component="echo", source_ref="no-such-source")


def test_invalid_plan_never_reaches_harness() -> None:
    """ValidationError is raised at Plan construction — harness is never invoked."""
    with pytest.raises(ValidationError):
        Plan(component="real-model", source_ref="syn-001")
...
def test_invalid_config_direct_construction_raises() -> None:
    ...
    with pytest.raises(ValidationError):
        Config(component="bad-component", source_ref="syn-001")
