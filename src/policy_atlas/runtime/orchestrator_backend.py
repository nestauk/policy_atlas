"""Orchestrator backend seam (task 024, decision 3) — router + watch moments.
...
from policy_atlas.evidence_base.assess.screen_prompt import SCREEN_MODEL
...
ORCHESTRATOR_MODEL = os.environ.get("POLICY_ATLAS_ORCHESTRATOR_MODEL", "gpt-5.5")
ORCHESTRATOR_TRIAGE_MODEL = os.environ.get(
    "POLICY_ATLAS_ORCHESTRATOR_TRIAGE_MODEL", SCREEN_MODEL
)
...
class OrchestratorBackend(Protocol):
    """The orchestrator seam for the router and watch moments.
...
class OpenAIOrchestratorBackend:
    """Live OpenAI implementation of the orchestrator seam.
    ...
    def __init__(
        ...
        self._client = resolve_openai_client(
            api_key,
            backend_name="OpenAIOrchestratorBackend",
            timeout=180.0,
            max_retries=2,
        )
    ...
    def triage(
        ...
        *,
        ...
        parsed = self._parse(
            ...
            model=ORCHESTRATOR_TRIAGE_MODEL,
            max_output_tokens=WATCH_TRIAGE_MAX_OUTPUT_TOKENS,
            usage_event="orchestrator.triage.usage",
            label="orchestrator-triage",
...
def _refuse_all_router() -> RouterCompileWire:
    ...
    from policy_atlas.runtime.orchestrator_prompt import RouterFragmentWire
...
class StubOrchestratorBackend:
    """Deterministic, zero-egress, scriptable orchestrator backend for tests/CLI.
...
def run_watch_decision(
    ...
    *,
    ...
    Args:
        backend: The orchestrator backend.
    ...
    for _round in range(WATCH_FALLBACK_TOOL_CALLS):
        ...
        try:
            ...
            log.warning("orchestrator.deliberation_read_failed", tool=tool, error=str(exc)[:200])
...
def build_watch_discretion_hook(
    ...
    *,
    ...
    rule (the authority order — declared rules > orchestrator — is enforced in the
    ...
    - ``choose_option`` / ``author`` → the watch's delta applied through the SAME
      apply machinery a standing rule uses, attributed to the orchestrator.
    ...
    def _hook(context: _DiscretionContext) -> _DiscretionOutcome:
        try:
            ...
            log.warning(
                "orchestrator.watch_hook_failed",
                steer_point=context.steer_point,
                error=str(exc)[:200],
            )
        ...
        profile = {"model": ORCHESTRATOR_MODEL, "prompt_version": WATCH_PROMPT_VERSION}
        if decision.action in {"choose_option", "author"} and decision.delta:
            return _DiscretionOutcome(
                interpreted_action="apply",
                rule="orchestrator_decision",
