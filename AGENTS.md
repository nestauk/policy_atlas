...
= the task id; **V10** (owner riders from `deferred.md`): `http_budget`→`call_budget`, delete dead `RunPane`/`JourneyPane`; **V11** (owner): the post-sign-in return-to fix (auth-adjacent, security lane reads it). **Tier 4** — table renames + a public-API path change (`/api/v1/tasks`,
...
lifecycle IA) and retires ADR 0031 decision 2 (the screen/code split). Eleven
numbered defects V1–V11 and the owner forks F1–F5 are in
