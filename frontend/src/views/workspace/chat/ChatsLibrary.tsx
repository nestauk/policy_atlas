...
import { addOpenChatTab, taskAgentConversationId, useActiveConversation, useConversationMutations } from "./conversationState";

type LibraryRow = components["schemas"]["ConversationListItemOut"];

/** Overlay listing a Task's chats, the Task Agent pinned first.
 *
 * Args:
 *   props: Task identity, overlay visibility controls, and
 *     `onOpenTaskAgent` — supplied on the Agent tab, where the Task Agent
 *     already owns the main column and must not open here as well.
 *
 * Returns:
 *   The active and archived conversation library when open.
 */
export function ChatsLibrary({ taskId, open, onClose, onOpenTaskAgent }: { taskId: string; open: boolean; onClose: () => void; onOpenTaskAgent?: () => void }) {
  ...
  const openRow = (row: LibraryRow) => {
    if (row.kind === "planning" && onOpenTaskAgent !== undefined) {
      onOpenTaskAgent();
      onClose();
      return;
    }
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Chats" className="absolute inset-x-3 top-3 z-20 max-h-[calc(100%-24px)] overflow-y-auto border border-line bg-paper p-4 shadow-lg">
      ...
      {pinned !== null && <LibraryRow row={pinned} {...rowProps} />}
      <LibraryGroups rows={rest} {...rowProps} />
      {(archived.data?.data.length ?? 0) > 0 && <><h3 className="mt-5 border-b border-line pb-1 text-meta font-bold uppercase tracking-[0.06em] text-grey">Archived</h3><LibraryGroups rows={archived.data?.data ?? []} {...rowProps} onOpen={openArchivedRow} onArchive={(id) => void unarchive(id)} archived /></>}
    </div>
  );
}

function LibraryGroups({ rows, ...props }: { rows: LibraryRow[]; taskAgentId: string; onOpen: (row: LibraryRow) => void; editing: string | null; title: string; setTitle: (value: string) => void; onRename: (row: LibraryRow) => void; onCommit: (id: string) => Promise<void>; onArchive: (id: string) => void; archived?: boolean }) {
