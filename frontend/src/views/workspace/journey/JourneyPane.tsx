...
function PlanRecap({ plan }: { plan: PlanDraft | null }) {
  ...
  return <Card className="mb-4 min-w-0">
    ...
    {open && <div className="border-t border-line px-4 pb-4 pt-3">
      {settings.length > 0 && <div className="grid gap-px border border-line bg-line sm:grid-cols-2">{settings.map(([label, value]) => <div key={label} className="bg-paper px-3 py-2"><p className="text-caption font-bold uppercase tracking-[0.06em] text-grey">{label}</p><p className="text-meta font-medium text-navy">{value}</p></div>)}</div>}
