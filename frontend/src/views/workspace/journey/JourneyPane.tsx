...
import { collapseActivity, completionCopy, FUNNEL_STAGES, funnelBarWidth, screenedOutFooter, timelineSummary } from "./presentation";
...
/** The landscape read model mixes ISO codes and country names. Combine them
 * before the compact publication-country distribution is displayed. */
function normaliseGeographies(data: Record<string, number>): Record<string, number> {
  return Object.entries(data).reduce<Record<string, number>>((normalised, [raw, count]) => {
    const clean = scrub(raw).trim();
...
export function JourneyPane({
  ...
  return (
    <div className="h-full overflow-y-auto px-5 py-5" style={{ scrollbarGutter: "stable" }}>
      {runStatus === "running" && <ProgressStrip stages={stream.stages} />}
      <PlanRecap plan={plan} />

      <nav aria-label="Journey sections" className="sticky top-0 z-20 -mx-5 mb-4 flex gap-3 overflow-x-auto border-b border-line bg-ground/95 px-5 py-2 backdrop-blur-sm">
        ...
        {(activeSearch || activity.length > 0) && <a className="shrink-0 text-[11px] font-bold uppercase tracking-wide text-navy hover:text-blue" href="#journey-activity">Activity</a>}
...
function PlanRecap({ plan }: { plan: PlanDraft | null }) {
  ...
  return <Card className="mb-4">
    <button className="flex w-full items-baseline gap-3 px-4 py-3 text-left hover:bg-ground" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label="Toggle plan recap">
      ...
      {plan.time_band !== null && plan.time_band !== "" && <Chip tone="soft">{scrub(plan.time_band)}</Chip>}
...
function CoverageCard({ coverage }: { coverage: Coverage }) {
  ...
  return <section id="journey-coverage" className="scroll-mt-14"><Card className="anim-rise p-4"><PaneHeading className="mb-3 p-0">Where I looked</PaneHeading>{details.length > 0 ? <div className="grid gap-3 sm:grid-cols-2">{details.map((backend) => <div key={backend.backend} className="border border-line p-3"><div className="flex items-baseline justify-between gap-2"><span className="text-[12px] font-bold text-navy">{backendLabel(backend.backend)}</span><span className="text-[11px] text-grey"><CountUp value={backend.results} className="font-display text-[17px] font-bold text-blue" /> results · <CountUp value={backend.relevant} className="font-display text-[17px] font-bold text-blue" /> relevant</span></div><div className="mt-2 max-h-28 space-y-1 overflow-y-auto">{(backend.queries ?? []).map((query, index) => <div key={`${query.query}-${index}`} className="flex gap-2 text-[11px]"><span className="min-w-0 flex-1 truncate italic text-grey">“{scrub(query.query)}”</span><span className="text-navy">{query.results}</span></div>)}</div></div>)}</div> : <p className="text-[12.5px] text-navy">{(coverage.backends ?? []).map(backendLabel).join(" · ")}</p>}<p className="mt-3 text-[12.5px] text-navy">{scrub(coverage.sentence)}</p><p className="mt-1 text-[11px] text-grey">Relevant counts are attributed across this project; per-query relevance was not recorded.</p></Card></section>;
}
...
function ActivityCard({ activity }: { activity: Array<{ stage: string; note: string }> }) {
  const lines = collapseActivity(activity).slice(-8);
...
function LandscapeEmbed({ landscape }: { landscape: Landscape }) {
  const distributions = [["Evidence types", landscape.evidence_types], ["Publication years", landscape.years], ["Where sources were published", landscape.geographies === null || landscape.geographies === undefined ? undefined : normaliseGeographies(landscape.geographies)]] as const;
  return <section className="scroll-mt-14"><Card className="anim-rise p-4"><PaneHeading className="mb-3 p-0">Evidence landscape</PaneHeading><div className="grid gap-4 lg:grid-cols-2">{distributions.map(([title, data]) => { const rows = Object.entries(data ?? {}).sort(([, a], [, b]) => b - a).slice(0, 5); if (rows.length === 0) return null; const maximum = rows[0][1] || 1; return <div key={title}><p className="mb-2 text-[12px] font-bold text-navy">{title}</p>{rows.map(([label, count]) => <div key={label} className="mb-1 flex items-center gap-2 text-[11px]"><span className="w-28 truncate text-right text-grey">{scrub(label)}</span><span className="h-2 flex-1 bg-ground"><span className="block h-full bg-blue" style={{ width: `${funnelBarWidth(count, maximum)}%` }} /></span><span className="w-5 font-bold text-navy">{count}</span></div>)}{title === "Where sources were published" && <p className="mt-2 text-[11px] text-grey">Where sources were published, not where the studies were conducted.</p>}</div>; })}</div></Card></section>;
}
