  ...
  return (
    <div className="h-full w-full min-w-0 max-w-full overflow-x-hidden overflow-y-auto px-5 py-5" style={{ scrollbarGutter: "stable" }}>
      ...
      <div className="space-y-5">
        <CompletionCard taskId={taskId} status={runStatus} funnel={funnel} />
        ...
        {checkIn}
        {hasLandscape && <LandscapeEmbed taskId={taskId} landscape={landscape!} />}
...
function StatusBanner({ status }: { status: string | undefined }) {
  if (status === "paused") return <p className="mb-4 border-l-[3px] border-orange bg-yellow-tint p-3 text-body text-navy">Paused at a check-in — waiting on you. The run continues when you answer.</p>;
  if (status === "degraded") return <p className="mb-4 border-l-[3px] border-orange bg-yellow-tint p-3 text-body text-navy">Completed with some flagged events — recorded in the decision log, not hidden.</p>;
  if (status === "aborted") return <p className="mb-4 border-l-[3px] border-orange bg-yellow-tint p-3 text-body text-navy">You stopped this run. Everything completed so far is kept below.</p>;
  if (status === "failed") return <p className="mb-4 border-l-[3px] border-red bg-red-tint p-3 text-body text-navy">This run failed. Whatever completed is kept and readable.</p>;
  return null;
}
...
function PlanRecap({ plan }: { plan: PlanDraft | null }) {
  ...
  return <Card className="mb-4 min-w-0">
    ...
    {open && <div className="border-t border-line px-4 pb-4 pt-3">
      {settings.length > 0 && <div className="grid gap-px border border-line bg-line sm:grid-cols-2">{settings.map(([label, value]) => <div key={label} className="bg-paper px-3 py-2"><p className="text-meta font-bold uppercase tracking-[0.06em] text-grey">{label}</p><p className="text-meta font-medium text-navy">{value}</p></div>)}</div>}
      ...
      {((plan.scoping_notes ?? []).length > 0 || (plan.screening_criteria ?? []).length > 0) && <div className="mt-3"><PaneHeading className="p-0">Screening rules</PaneHeading>{(plan.scoping_notes ?? []).length > 0 && <div className="mt-1 flex flex-wrap gap-1.5">{plan.scoping_notes?.map((note) => <Chip key={note} tone="soft">{scrub(note)}</Chip>)}</div>}{(plan.screening_criteria ?? []).length > 0 && <ul className="mt-1 space-y-1 text-body text-navy">{plan.screening_criteria?.map((criterion) => <li key={criterion}>• {scrub(criterion)}</li>)}</ul>}</div>}
      {(plan.steps ?? []).length > 0 && <div className="mt-3"><PaneHeading className="p-0">Agreed steps</PaneHeading><ol className="mt-1 space-y-1 text-body text-navy">{plan.steps?.map((step, index) => <li key={step.stage}>{index + 1}. {scrub(step.label)}{step.blurb !== "" && <span className="text-grey"> — {scrub(step.blurb)}</span>}</li>)}</ol></div>}
      {(plan.assumptions ?? []).length > 0 && <div className="mt-3"><PaneHeading className="p-0">Assumptions</PaneHeading><ul className="mt-1 space-y-1 text-body text-grey">{plan.assumptions?.map((assumption) => <li key={assumption}>• {scrub(assumption)}</li>)}</ul></div>}
    </div>}
  </Card>;
}
...
function Timeline({ stages, plan }: { stages: StageEntry[]; plan: PlanDraft | null }) {
  ...
  if (rows.length === 0) return <p className="text-body text-grey">Stages will appear as the analysis begins.</p>;
...
function FunnelCard({ funnel }: { funnel: Funnel }) {
  ...
  return <section id="journey-funnel" className="scroll-mt-14"><Card className="anim-rise p-4"><PaneHeading className="mb-3 p-0">From sources to evidence</PaneHeading><div className="space-y-2">{rows.map((row) => <Tooltip key={row.key} content={<p className="text-body text-navy">{row.definition}</p>}><div className="flex items-center gap-3"><span className="w-28 shrink-0 text-right text-caption font-medium text-navy">{row.label}</span><span className="h-3 flex-1 bg-ground"><span className="anim-bar block h-full bg-blue" style={{ width: `${funnelBarWidth(row.value, max)}%` }} /></span><CountUp value={row.value} className="w-8 shrink-0 text-meta font-bold text-navy" /></div></Tooltip>)}</div></Card></section>;
}

function CoverageCard({ coverage }: { coverage: Coverage }) {
  const details = coverage.backends_detail ?? [];
  return <section id="journey-coverage" className="scroll-mt-14"><Card className="anim-rise min-w-0 p-4"><PaneHeading className="mb-2 p-0">Where I looked</PaneHeading>{details.length > 0 ? <><p className="mb-3 break-words text-body text-grey">{/* Task 031: the two numbers count different things and neither contains the other — hits are per search call and include duplicates, kept sources are counted once across every round. */}Query hits count every result a search returned, across all rounds, before duplicates are removed. Kept sources are counted once each.</p><div className="grid min-w-0 gap-3 sm:grid-cols-2">{details.flatMap((backend) => backendLabel(backend.backend) === null ? [] : [<div key={backend.backend} className="min-w-0 border border-line p-3"><div className="flex min-w-0 items-baseline justify-between gap-2"><span className="min-w-0 break-words text-caption font-bold text-navy">{backendLabel(backend.backend)}</span><span className="text-right text-caption text-grey"><CountUp value={backend.results} className="font-display text-body font-bold text-blue" /> query hits · <CountUp value={backend.relevant} className="font-display text-body font-bold text-blue" /> kept after screening</span></div><div className="mt-2 max-h-28 min-w-0 space-y-1 overflow-y-auto">{(backend.queries ?? []).map((query, index) => <div key={`${query.query}-${index}`} className="flex min-w-0 gap-2 text-meta"><span className="min-w-0 flex-1 truncate italic text-grey">“{scrub(query.query)}”</span><span className="shrink-0 text-navy">{query.results}</span></div>)}</div></div>])}</div></> : <p className="break-words text-caption text-navy">{(coverage.backends ?? []).map(backendLabel).filter((label): label is string => label !== null).join(" · ")}</p>}</Card></section>;
}

function CompletionCard({ taskId, status, funnel }: { taskId: string; status: string | undefined; funnel?: Funnel }) {
  ...
  return <Card className={`anim-rise border-l-[3px] p-5 ${status === "degraded" ? "border-l-orange" : "border-l-green"}`}><PaneHeading className="mb-2 p-0">Done</PaneHeading><h3 className="font-display text-heading font-semibold text-navy">{copy.heading}</h3>{counts.length > 0 && <p className="mt-1 text-meta text-navy">{counts.join(", ")}</p>}<div className="mt-4 flex flex-wrap gap-3"><Link className="cutout bg-blue px-3 py-2 text-meta font-bold text-white" to={`/tasks/${taskId}/results`}>Read the evidence base</Link><Link className="border border-line-2 bg-paper px-3 py-2 text-meta font-bold text-navy" to={`/tasks/${taskId}/sources/all`}>All sources</Link></div></Card>;
}

function GroupsCard({ groups }: { groups: Groups }) {
  const facets = groups.facets ?? [];
  return <section id="journey-groups" className="scroll-mt-14"><Card className="anim-rise p-4"><PaneHeading className="mb-3 p-0">Findings by group</PaneHeading><div className="space-y-4">{facets.map((facet) => { const top = [...(facet.groups ?? [])].sort((a, b) => b.size - a.size).slice(0, 8); const maximum = top[0]?.size ?? 1; return <div key={facet.facet}><p className="mb-1.5 text-meta font-bold uppercase tracking-[0.06em] text-grey">{scrub(facet.facet)}</p>{top.map((group) => <div key={group.label} className="mb-1 flex items-center gap-2"><Tooltip content={<p className="text-body text-navy">{scrub(group.description)}</p>}><span className="w-32 shrink-0 truncate text-right text-caption font-medium text-navy">{scrub(group.label)}</span></Tooltip><span className="h-2 flex-1 bg-ground"><span className="anim-bar block h-full bg-blue" style={{ width: `${funnelBarWidth(group.size, maximum)}%` }} /></span><span className="w-5 text-meta font-bold text-navy">{group.size}</span></div>)}</div>; })}</div></Card></section>;
}

function LandscapeEmbed({ taskId, landscape }: { taskId: string; landscape: Landscape }) {
