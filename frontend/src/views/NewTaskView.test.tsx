...
describe("NewTaskView — task selector", () => {
  ...
  it("offers a colleague-owned, org-visible task — colleague assignment (owner ruling 2026-08-27)", async () => {
    const user = userEvent.setup();
    vi.mocked(queries.useProjects).mockReturnValue(
      {
        data: {
    ...
    renderNewTask("/new?capability=evidence_search");
    await user.click(screen.getByLabelText(/Add to a task/));
    expect(screen.getByRole("option", { name: "A colleague's task" })).toBeInTheDocument();
  });

  it("keeps the task selector when every task is colleague-owned", () => {
    vi.mocked(queries.useProjects).mockReturnValue(
      {
        data: {
          data: [
    ...
    renderNewTask("/new?capability=evidence_search");
    expect(screen.getByLabelText(/Add to a task/)).toBeInTheDocument();
  });
});
