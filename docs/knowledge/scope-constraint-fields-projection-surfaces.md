---
type: Invariant
title: A new ScopeConstraints field must touch every projection surface — and the Task Agent sees it via the wire draft, not the CLI render
...
|---|---|---|
| `PlanDraftWire` (`runtime/task_agent_prompt.py`) | The Task Agent's ONLY view of current constraints on re-planning turns (the draft attachment). | The Task Agent cannot see or preserve the field. NB `_render_scope_constraints` (`runtime/agent.py`) does NOT reach the Task Agent — its only consumer is the dev CLI approval prompt (`agent.py` `console.print(_render_full_plan(...))`). The 039 build recorded the CLI renderer as the mechanism; the step-7 contract verifier corrected it. |
| `_draft_from_wire` (`api/routers/task_agent.py`) | Folds the loose wire (`str`) into the strict API draft (`Literal[...]`). | An unguarded `model_validate` 500s the whole Task Agent turn on a sloppy planner value (e.g. `"APO"` for `"apo"`) — normalise the taught spellings and drop anything else so the turn degrades (`ready=false`) instead (039 review stack, confirmed). |
...
- `backend/src/policy_atlas/runtime/task_agent_prompt.py` (`PlanDraftWire`)
- `backend/src/policy_atlas/api/routers/task_agent.py` (`_draft_from_wire` normaliser)
- `backend/tests/api/test_task_agent_router.py::test_draft_from_wire_normalises_loose_publisher_source`
