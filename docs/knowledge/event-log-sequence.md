...
description: event_log.sequence is assigned max+1 in app code; ordering and append-only live at the repo layer. Since 025, two unserialized writer families exist (the walk executor and project-locked API mutations), so a sequence collision retries under a SAVEPOINT rather than failing the caller's transaction; run_id became nullable for run-less project lifecycle audit events.
...
Since 025, `events.append` retries under a **SAVEPOINT** on `(project_id, sequence)` collision: two
unserialized writer families now exist per project — the walk executor and the API's project-locked
...
Also since 025: `event_log.run_id` is **nullable** (owner-approved gate expansion). Run-less project
lifecycle audit events (`project.renamed`, `project.archived`) attach no run — they happen outside any
...
`max+1` is correct **only under a serial single writer** for any one project — the 025 SAVEPOINT retry
is what makes "single writer per project at a time" hold under two writer families that aren't
