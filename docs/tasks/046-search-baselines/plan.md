...
> folded into S1, S4–S6 and Phase 3 below. The P2 deferral (owner, 2026-09-25) then
> removed the key-suggestion phase and the raw Overton arm.
...
holds three functions with one signature: `fetch_<arm>(intent, cutoff, cap, *, get) ->
...
credit price is filled in from the owner's plan in the Phase 3 preflight; until then it
...
with `meta` per D6. The task output also carries `n_ground_truth` for the printed lines;
it is not uploaded as a score.

S5. **Printed summary only, no CSV.** After all runs the script prints one table: arm,
cap, mean recall, total candidates kept, total HTTP requests, failed requests, total
`api_cost_usd`, and the Langfuse run URL. Per review, one line during the run in the style
of `production_recall._describe`.
...
Consensus needs the key and is probed by the lead in Phase 3.
...
## Phase 3 — Preflight, live run and history rows: P1, P3, P4 — `lead`

Needs the owner's keys, spends money, and the notes are judgment. Order matters: nothing
live runs before step 2 passes.

1. Read the Consensus plan page: included credits this month, credit price, largest
   `page_size`, and any stated rate limit. Fill the price table (S3) and the page size.
2. **Spend and time preflight.** Requests for the full run = 4 reviews × pages for caps
...
3. `baseline_recall.py` over all arms and caps. Expected wall time: Semantic Scholar about
   1 minute; OpenAlex under 1 minute; Consensus as measured in step 2.
4. `history.py --since <today>` → copy rows into `results/history.md` with notes: D1
   favours relevance-ranked services; D2 Consensus month rounding; the target is scholarly
   only (P2 deferred); the comparison rows are the 2026-09-22 pipeline rows at commit
   `b16f859` (D8); the raw-versus-pipeline comparison is a sign, not a controlled test;
   any failed requests; what the cost column leaves out.
5. README section: what the baselines are, how to run them, how to read them next to the
   pipeline rows, and what the cost column does and does not include.
6. Commit.
...
and the suggest-only key lookup design at commit `37d496c`; the "swap" slice) in
`docs/deferred.md`. Commit. Stop. Review runs in a fresh conversation.
