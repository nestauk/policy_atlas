...
7. [ ] Known gaps and deferred seams listed (gap → [docs/deferred.md](../../deferred.md)):
       EB artefact composition · very-large-corpus grouping (discovery-sampling /
       embedding-based clustering) · grouping-quality + adversarial-content evals ·
       steer-point pause · dual-view coverage · pgvector/retrieval (chunk vectors'
       first reader) · Bedrock swap · exact-token budgeting · v2 `group`-seam lessons ·
       tag consolidation; class-1 vectorisation entry marked discharged-ahead-of-reader
       (approved exception).
