...
1. [ ] **Start the longlist.** `confirm_plan` at the gate and
   ...
   row; the walk ends `succeeded`, `degraded` (a child failed) or `failed`
   (a spine step failed, Result stays on the baseline with the failure
   line); "Search further" in words is refused honestly; an ES task has no
   longlist chain.
...
3. [ ] **The chain and the pool.** The chain suggest → option searches ∥
   ...
   classify and appraise skip resolved rows; the broad acquire uses the
...
4. [ ] **Suggestions and entrants, each with an option search.** The
   ...
   longlist walk with the user's and the report's never dropped; a failing
   child degrades the parent; an entrant whose search finds nothing
...
11. [ ] **System records and API.** The longlist routes exist with
    ...
    only through an existing link); the confirm surfaces return the opened
    walk in an optional field; `your_options` and the default preference
    are on the plan bodies; child walks appear on the run stream; the
    OpenAPI diff is additive and regenerated by `make openapi-sync`.
