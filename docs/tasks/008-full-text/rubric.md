...
1. [ ] Implementation satisfies [contract.md](contract.md) (rev 5).
2. [ ] `make verify` passes (test · typecheck · lint · build); all checks deterministic
       (pinned parser versions; fixture replay).
3. [ ] No approval-gated change snuck in unapproved — schema limited to the three approved
       columns on `project_source_snapshot`; dependencies limited to `pymupdf4llm`
       (+`pymupdf`) + `trafilatura`; public interface limited to the `document_fetcher`
       parameter + registry entry; **zero runtime egress** (fixture replay only; parsers
       are model-free local code; recorder script never imported by the package); no
       auth/CI/production config change.
...
6. [ ] Verification evidence recorded ([verification.md](verification.md)), including the
       ...
       no-truncation long-report and fan-out determinism results, and the measured
       long-document wall-clock/peak-memory numbers.
7. [ ] Known gaps and deferred seams listed in [docs/deferred.md](../../deferred.md):
       ...
       retry/backoff) · ML-layout parse escalation (docling) · time-budget-aware parser
       selection · chunk-volume bias controls at the retrieve seam (per-document
