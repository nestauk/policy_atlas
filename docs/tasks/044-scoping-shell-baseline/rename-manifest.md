# Rename manifest (task 044, phase one)

Deliverable 2 of the [044 contract](contract.md): the inventory the plan phase
...
listed so the plan phase knows which revision the 044 migration must
...
| Merged task docs under `docs/tasks/` other than 044 | e.g. `docs/tasks/038-vocabulary-alignment/**`, `docs/tasks/027-frontend-port/**` | historical record |
...
| `scripts/rename_038.py` (3 occurrences: `test_planning_transcript_migration.py` filename ref, `planner_prompt.py` path ref, a comment) | historical 038 rename tooling | historical tooling; per plan S9 it becomes a thin caller of the shared engine with its own rule table untouched, and is never re-run |
...
- Excluded by path (never touched): `docs/adr/**`, `docs/tasks/**` (other than 044), `docs/specs/sources/**`, `docs/specs/log.md`, `frontend/openapi.json`, `frontend/src/api/gen/types.ts`, `infra/cdk.out`, `scripts/rename_038.py`, three gitignored files.
...
| `docs/tasks/**` other than `_templates/` and `044-scoping-shell-baseline/` | historical record |
