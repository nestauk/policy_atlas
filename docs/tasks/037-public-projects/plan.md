...
3. `contract/projects.py`: `is_public: bool` on `ProjectOut`;
   `is_public: bool | None = None` on `ProjectUpdate`. No new combination
   rules — `is_public` is orthogonal to `visibility` and `portfolio_ids`
   (D1).
...
5. Tests: schema round-trip (`tests/core/test_schema.py` column set);
   PATCH owner 200 / colleague 403 / anonymous 401; audit event on flip and
   not on a no-op PATCH; `is_public` present in `ProjectOut`.
...
1. `api/auth.py` — `get_optional_user`: missing credentials → `None`;
   present credentials → delegate to `get_current_user` unchanged (so a
   bad token still 401s, D2). Re-export through `deps.py`.
...
5. Tests: anonymous route renders Results for a public Task (mock fetch);
   private Task redirects to splash with the URL stashed; tab set is
   exactly two; signed-in behaviour unchanged (`App.tsx` still swaps
   routers on `auth.status`).
...
- The step-4 ADR (0034: public read leg via `is_public`, D1/D2) is written
  when the owner approves the contract, before the build opens.
