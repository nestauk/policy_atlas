...
| 13 | `BlockOut.gaps` | **Populate the declared-but-empty field** (grade + caveat incl. `verdict_origin`) | `annotation.payload["gap"]` | **[D‑9]** an always-`[]` declared field is dishonest either way; the claim-panel gap explainer needs it ("coverage claims keep their base"). |
...
- **[D‑1] Live search card — built with zero backend change**: `search.executed`
  decisions are already public with `query`, `result_count`, `backend`; the
  card polls the decisions read (TanStack refetch during acquire, tick-paced)
  and groups client-side; the coverage card takes over at stage end. Per-query
  relevance and mid-run per-backend *relevant* counts are honestly omitted
  (never recorded / end-of-stage only). No new endpoint, no SSE change.
