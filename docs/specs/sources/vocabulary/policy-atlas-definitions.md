...
> Frozen source (ADR 0002). Supplied by the owner on 2026-09-04 as "Policy Atlas: Definitions".
> The living glossary is `docs/specs/system/vocabulary.md`. Text below is verbatim except for
> markdown normalisation; the bracketed status words are the team's.
...
* **Agent:** Users start their work by interacting with the Agent (we call it the "orchestrator" in the backend), which helps with **Planning** the work. The Agent creates **Plans** that the user can inspect and edit before running long workflows, and allows the user to **Ask questions** about the completed work.
...
**Agent overlay**: chat interface (sidebar) that allows users to **Plan** the task or **Ask questions** about the Task or Project Artefacts. This should also be the main surface through which the user gets "process updates" (for example, when something is running). Agent has different modes:

- **Planning mode**: asking questions to the user and setting up a piece of work
- **Agent mode**: while a task is running.
- **Questions mode:** answering questions about artefacts, after a run has completed
