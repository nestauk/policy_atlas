...
stands (tasks 001–010 — backend only; setup unchanged since 002; task 007 adds two dev-time
...
`LANGFUSE_HOST`** enable full-I/O tracing; task 010 adds **no new deps or env vars** — the
same `OPENAI_API_KEY` live route now also drives the select rerank; `make verify` and the
