...
from policy_atlas.evidence_search.synthesis.synthesis_tools import SECTION_CAP
...
# EVIDENCE_SEARCH_COVERAGE / DEEPENING_SELECTION / SYNTHESIS_SHAPE); the registry
...
class ScopeConstraints(BaseModel):
    ...
    def to_filters(self) -> dict[str, dict[str, Any]]:
        ...
        Returns:
            ...
            geography under ``overton``, author-affiliation geography under
            ``openalex``, and the ``publisher_source`` Overton
            source-collection filter also under ``overton``. Empty
...
def _directive_delta(component: str, plan: TaskPlan) -> dict[str, Any]:
