...
from policy_atlas.evidence_search.group.group_clustering import OpenAIGroupClusteringBackendFactory
from policy_atlas.evidence_search.sourcing import search_generation, search_live
from policy_atlas.evidence_search.sourcing.acquire import SearchBackend
from policy_atlas.evidence_search.sourcing.country_filters import TIER1_GROUPS, expand_tier1
...
class ConsoleIO(Protocol):
    ...
    def print(self, message: str) -> None:
        """Print one line of agent output."""
        ...
...
class StdConsole:
    """Real stdin/stdout console used by ``python -m policy_atlas.runtime.agent``."""
...
@dataclass
class AgentResult:
    """Structured result of one agent session (testability seam).
    ...
    Args:
        ...
        conversation_id: Langfuse session id minted for this agent conversation.
    """
...
class CliIO:
    ...
    def pause(self, point: dict[str, Any], render: str) -> SteeringResponse:
        ...
        while True:
            ...
            for option in options:
                ...
                num += 1
            if authored:
                lines.append("Suggested for this run (suggested by the agent):")
    ...
    def _authored_to_response(self, option: dict[str, Any]) -> SteeringResponse:
        ...
        agent authored the option; the user picked it — the decision
        event records decided_by=user, authored_by=agent (discipline iv).
        """
        ...
        if not isinstance(component, str) or not component or not delta:
            return Continue()
        return Adjust(directive_deltas={component: delta}, authored_by="agent")
...
def _render_scope_constraints(plan: TaskPlan) -> list[str]:
...
def _seed_stub_corpus(engine: Engine, task_id: uuid.UUID) -> None:
    ...
    Args:
        ...
        task_id: The task the corpus is ingested into.
    """
...
def persist_approved_plan(
    ...
    *,
    ...
    latest_version = conn.execute(
        sa_select(func.coalesce(func.max(task_plan.c.version), 0)).where(
...
def _write_plan_row(
    ...
    *,
    ...
    """Create task + evidence scope and insert the approved plan row.
    ...
    with engine.begin() as conn:
        ...
        scope_id, plan_id = persist_approved_plan(conn, task_id=task_id, plan=plan)
    return task_id, scope_id, plan_id


def _artefact_present(engine: Engine, task_id: uuid.UUID) -> bool:
...
def main(
    ...
    """Run one agent session: plan -> approve -> run -> report.
    ...
    if not live:
        _seed_stub_corpus(engine, task_id)
    ...
    artefact_present = _artefact_present(engine, task_id)
