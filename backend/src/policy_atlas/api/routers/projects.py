...
def _cascade_visibility(
    ...
    **Archived members are included** (contract § 6, i.4). They are excluded
    ...
    after the owner made the Task private.
...
def _task_counts(
    ...
    colleague the true size of a Task whose private Tasks they cannot
...
def update_project(
    ...
    **Two code paths, not one.** `name` and `description` are written by an
    ...
    produces is silent — the owner sets a Task private, the UI agrees, and
    ...
    **Owner-only, like every other write** (contract § 3): the route resolves
    ...
    leave an org-visible row inside a private Task.
