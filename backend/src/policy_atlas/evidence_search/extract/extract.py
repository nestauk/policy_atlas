...
class _Doc:
    ...
    # uq_ser_memo (task_id, source_snapshot_id, extraction_fingerprint).
...
def _load_docs(
    ...
    *,
    ...
    for record in selected:
        raw_id = record.get("tss_id")
        if not isinstance(raw_id, str):
            raise ExtractError("selected record missing a string tss_id")
        try:
            tss_id = uuid.UUID(raw_id)
        except ValueError as exc:
            raise ExtractError(f"selected record carries an invalid tss_id: {raw_id!r}") from exc
        text_basis = record.get("text_basis")
        order.append((tss_id, text_basis if isinstance(text_basis, str) else ""))

    tss_ids = [tss_id for tss_id, _ in order]
    ...
    if tss_ids:
        rows = conn.execute(
            ...
            .where(task_source_snapshot.c.task_source_snapshot_id.in_(tss_ids))
        ).fetchall()
        for row in rows:
            loaded[cast("uuid.UUID", row.task_source_snapshot_id)] = {
    ...
    docs: list[_Doc] = []
    for tss_id, text_basis in order:
        row_data = loaded.get(tss_id)
        if row_data is None:
            raise ExtractError(f"selected tss {tss_id} has no task_source_snapshot row")
...
def _assert_invariants(docs: list[_Doc], *, selected_count: int) -> None:
    ...
    tss_ids = [doc.tss_id for doc in docs]
    if len(tss_ids) != selected_count or len(set(tss_ids)) != selected_count:
        raise ExtractError(
...
def _build_summary(
    ...
    *,
    ...
    for run in profile_runs:
        ...
        profile_docs_by_id[run.profile_id] = {doc.tss_id: doc for doc in run.docs}
    ...
    for doc in base_docs:
        ...
        for run in profile_runs:
            profile_doc = profile_docs_by_id[run.profile_id][doc.tss_id]
    ...
    *,
    ...
    try:
        ...
        digests = _load_relevance_digests(
            conn, task_id=task_id, iof_record_ids=iof_ids, icf_record_ids=icf_ids
        )
