...
def _load_stage1_docs(
    ...
    *,
    ...
    rows = conn.execute(
        query.order_by(task_source_snapshot.c.ingested_at)
    ).fetchall()
    ...
    *,
    ...
    for row in rows:
        ...
        candidate_records.append(
            (
                cast("uuid.UUID", row.task_source_snapshot_id),
    ...
    if rescreen:
        _assert_no_stage2_collision(
            ...
            candidates=[
                (tss_id, generation)
                for tss_id, _, _, _, generation in candidate_records
            ],
        )
    ...
    for tss_id, source_snapshot_id, chunk_snapshot_id, metadata, generation in (
        candidate_records
    ):
...
def _assert_no_stage2_collision(
    ...
    *,
    ...
    if not candidates:
        return
    existing = {
        (cast("uuid.UUID", row.task_source_snapshot_id), int(row.screen_generation))
    ...
    for tss_id, generation in candidates:
        if (tss_id, generation) in existing:
            raise ScreenSupersessionError(
                ...
                f"source {tss_id}; stage-2 supersession is not built — halt and "
...
def _run_stage2(
    ...
    *,
    ...
    docs, skipped_no_fulltext = _load_stage2_docs(
        conn, task_id=task_id, scope_id=context.scope_id, rescreen=rescreen
    )
    payloads: dict[int, ScreenFullTextPayload] = {}
...
def screen_sources(
    ...
    *,
    ...
    """Screen task sources against a scope.
