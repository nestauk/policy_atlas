...
from policy_atlas.evidence_search.corpus.characterise import ScreenedSource, screened_sources
...
from policy_atlas.evidence_search.corpus.theme_grouping import UNCLUSTERED, GroupingDoc
...
class SelectionCandidate:
    ...
    Attributes:
        ...
        tags: Scope/task source tags for boost and priority matching.
    """
...
class SelectionDirective:
    ...
    def normalized(self) -> dict[str, Any]:
        ...
        return {
            "budget": self.budget,
            "must_include_ids": [str(tss_id) for tss_id in self.must_include_ids],
            "boosts": [
            ...
            "exclude_ids": [str(tss_id) for tss_id in self.exclude_ids],
        }
...
def _signal_docs(
    ...
    *,
    ...
    for candidate in candidates:
        ...
        docs[candidate.source.tss_id] = _SignalDoc(
            candidate=candidate,
            signals=signals,
...
def _doc_for_rerank(signal_doc: _SignalDoc) -> GroupingDoc:
    ...
    return {"id": str(source.tss_id), "title": title, "abstract": abstract}
...
def _fallback_batch(
    ...
    for tss_id in batch:
        rank_infos[tss_id] = _RankInfo(score=None, reason=None, rank_fallback=True)
    return len(batch)
...
def _apply_ranked_output(
    *,
    ...
    valid = validate_ranked({str(tss_id) for tss_id in batch}, output)
    fallback_count = 0
    for tss_id in batch:
        ranked = valid.get(str(tss_id))
        if ranked is None:
            rank_infos[tss_id] = _RankInfo(score=None, reason=None, rank_fallback=True)
            fallback_count += 1
        else:
            rank_infos[tss_id] = _RankInfo(
                score=ranked["score"],
...
def _rerank_infos(
    *,
    ...
    title_only_count = sum(
        1
        for tss_id in contested_ids
        if _doc_for_rerank(signal_docs[tss_id])["abstract"] is None
    )
    log.info(
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_RERANK_BATCHES) as executor:
        for batch in batches:
            if not budget.reserve():
                ...
                continue
            docs = [_doc_for_rerank(signal_docs[tss_id]) for tss_id in batch]
            submitted.append((
                batch,
            ...
            if budget.reserve():
                ...
                try:
                    output, usage = ranking_backend.rank(
                        [_doc_for_rerank(signal_docs[tss_id]) for tss_id in batch],
                        intent=intent,
                    )
                    usage_totals.add(usage)
...
def _deterministic_order(
    ...
    return sorted(ids, key=lambda tss_id: (-signal_docs[tss_id].composite, str(tss_id)))
...
def _llm_order(
    ...
    scored = [
        tss_id
        for tss_id in ids
        if tss_id in rank_infos and rank_infos[tss_id].score is not None
    ]
    fallback = [
        tss_id
        for tss_id in ids
        if tss_id not in rank_infos or rank_infos[tss_id].score is None
    ]
    return sorted(
        scored,
        key=lambda tss_id: (
            -cast("int", rank_infos[tss_id].score),
            -signal_docs[tss_id].composite,
            str(tss_id),
        ),
...
def _priority_matches_stratum(
    ...
    return any(
        ...
        for tss_id in stratum.candidate_ids
        for tag in signal_docs[tss_id].candidate.tags
    )
...
def _selected_record(
    *,
    ...
    if strategy == "llm_rerank_v1" and tss_id in rank_infos:
        rank_info = rank_infos[tss_id]
...
def _stratum_share(
    ...
    candidate_full_text = sum(
        1 for tss_id in ids if signal_docs[tss_id].candidate.source.text_basis == "full_text"
    )
    selected_full_text = sum(
        1
        for tss_id in selected_ids
        if signal_docs[tss_id].candidate.source.text_basis == "full_text"
    )
...
def _flags(
    *,
    ...
    selected_ids = [
        ...
        for tss_id in selected_ids_for_stratum
    ]
...
def _assert_outcome_invariants(
    *,
    ...
    selected_ids = [record["tss_id"] for record in selected]
...
def select_documents(
    ...
    *,
    ...
    candidate_by_id = {candidate.source.tss_id: candidate for candidate in candidates}
    ...
    must_include_out_of_scope = [
        str(tss_id)
        for tss_id in directive.must_include_ids
        if tss_id not in selectable_ids
    ]
    notable_exclusions = [
        {"tss_id": tss_id, "flag": "must_include_not_in_scope"}
        for tss_id in must_include_out_of_scope
    ]
    selected_must_by_stratum = {stratum.name: set[uuid.UUID]() for stratum in ordered_strata}
    for tss_id in directive.must_include_ids:
        if tss_id in selectable_ids:
            selected_must_by_stratum[stratum_by_doc[tss_id]].add(tss_id)
    ...
    rankable_by_stratum = {
        stratum.name: [
            tss_id
            for tss_id in stratum.candidate_ids
            if tss_id not in selected_must_by_stratum[stratum.name]
        ]
        for stratum in ordered_strata
    }
    ...
    if strategy == "llm_rerank_v1":
        contested_ids = [
            ...
            for tss_id in sorted(rankable_by_stratum[stratum.name], key=str)
        ]
    ...
    for stratum in ordered_strata:
        ...
        for tss_id in must_ids:
            selected.append(
                _selected_record(
                    tss_id=tss_id,
                    stratum_name=stratum.name,
                    reason="must_include",
                    signal_doc=signal_docs[tss_id],
                    strategy=strategy,
                    rank_infos=rank_infos,
                )
            )
        ...
        for offset, tss_id in enumerate(selected_rankable):
            reason = "breadth_floor" if offset < allocation.floor_slots[stratum.name] else "ranked"
            selected_by_stratum[stratum.name].append(tss_id)
            selected.append(
                _selected_record(
                    tss_id=tss_id,
                    stratum_name=stratum.name,
    ...
    for stratum in ordered_strata:
        ...
        strata_summary.append({
            ...
            "selected_ids": [str(tss_id) for tss_id in selected_ids],
...
def _tags_by_source(
    ...
    *,
    ...
    for row in rows:
        tss_id = cast("uuid.UUID", row.task_source_snapshot_id)
        tags.setdefault(tss_id, []).append(
            SourceTag(
    ...
    return {tss_id: tuple(values) for tss_id, values in tags.items()}


def _candidate_inputs(
    sources: Sequence[ScreenedSource],
    tags_by_tss: dict[uuid.UUID, tuple[SourceTag, ...]],
) -> list[SelectionCandidate]:
    return [
        SelectionCandidate(source=source, tags=tags_by_tss.get(source.tss_id, ()))
...
def _load_strata(
    ...
    *,
    ...
    return [
        SelectionStratum(
            name=name,
            candidate_ids=tuple(tss_id for tss_id in ids if tss_id in eligible_ids),
        )
    ...
    *,
    ...
    return {
        ...
        "excluded": {
            ...
            "notable": [
                {"tss_id": str(tss_id), "flag": "excluded_by_directive"}
                for tss_id in excluded_by_directive_ids
            ],
        },
        "base": {
...
def select_scope(
    ...
    *,
    ...
    candidates = screened_sources(conn, task_id=task_id, scope_id=context.scope_id)
    non_evidence_free = [
        source
    ...
    excluded_by_directive_sources = [
        source for source in non_evidence_free if source.tss_id in exclude_ids
    ]
    eligible_sources = [
        source for source in non_evidence_free if source.tss_id not in exclude_ids
    ]
    ...
    if not eligible_sources:
        return _empty_summary(
            ...
            excluded_by_directive_ids=[source.tss_id for source in excluded_by_directive_sources],
        )

    tags_by_tss = _tags_by_source(
        conn,
        task_id=task_id,
        tss_ids=[source.tss_id for source in eligible_sources],
    )
    pure_candidates = _candidate_inputs(eligible_sources, tags_by_tss)
    eligible_ids = {source.tss_id for source in eligible_sources}
    strata = _load_strata(
        conn,
        task_id=task_id,
    ...
    excluded = {
        ...
        "notable": outcome.notable_exclusions
        + [
            {"tss_id": str(source.tss_id), "flag": "excluded_by_directive"}
