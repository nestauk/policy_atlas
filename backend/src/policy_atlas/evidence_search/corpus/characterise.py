...
# rotate: theme_id = uuid5(ns, f"{task_id}:{theme_name}").
...
def _base_counts(conn: Connection, *, task_id: uuid.UUID, scope_id: uuid.UUID) -> dict[str, int]:
    ...
    # task_sources bug raw rows caused.
    screen_failed = int(
        conn.execute(
            select(func.count(func.distinct(source_screening_result.c.task_source_snapshot_id)))
            ...
            .where(
                ~exists().where(
                    ...
                    & (effective.c.task_id == task_id)
                    & (
                        effective.c.task_source_snapshot_id
                        == source_screening_result.c.task_source_snapshot_id
                    )
                )
            )
        ).scalar_one()
    ...
    return {
        ...
        "unscreened": (
            task_sources - screened_in - not_relevant - excluded_retracted - screen_failed
        ),
    }
...
def screened_sources(
    ...
    *,
    ...
    """Load relevant screened sources in deterministic task-source order.
    ...
    # per (scope, tss) — never a raw source_screening_result join, which would
    ...
    rows = conn.execute(
        ...
        .select_from(
            ...
            .outerjoin(
                ...
                & (source_classification_result.c.task_id == effective.c.task_id)
                & (
            ...
            .outerjoin(
                ...
                & (source_appraisal_result.c.task_id == effective.c.task_id)
                & (
...
def _failed_embedding_count(conn: Connection, sources: list[ScreenedSource]) -> int:
    snapshot_to_tss: dict[uuid.UUID, set[uuid.UUID]] = {}
    for source in sources:
        snapshot_to_tss.setdefault(source.source_snapshot_id, set()).add(source.tss_id)
        if source.full_text_snapshot_id is not None:
            snapshot_to_tss.setdefault(source.full_text_snapshot_id, set()).add(source.tss_id)
    if not snapshot_to_tss:
        return 0
    ...
    rows = conn.execute(
        ...
        .where(chunk_table.c.source_snapshot_id.in_(list(snapshot_to_tss)))
        .where(missing_embedding_filter)
    )
    failed_tss_ids: set[uuid.UUID] = set()
    for (snapshot_id,) in rows:
        failed_tss_ids.update(snapshot_to_tss[snapshot_id])
    return len(failed_tss_ids)
...
def _coverage(
    ...
    *,
    ...
    sources = screened_sources(conn, task_id=task_id, scope_id=context.scope_id)
    tss_ids = [source.tss_id for source in sources]
    ...
    distributions = {
        ...
        "tags": _tag_distribution(conn, task_id=task_id, tss_ids=tss_ids),
    }
    rates = {
    ...
    return (
        {
            "base": "screened",
            "base_counts": _base_counts(conn, task_id=task_id, scope_id=context.scope_id),
...
def characterise_scope(
    ...
    *,
    ...
    coverage, sources = _coverage(conn, task_id=task_id, context=context)
    ...
    # Content-keyed (task, name) identity: deterministic across re-runs so
    # stub runs stay byte-identical and a re-characterise that keeps a theme's
    # name keeps its id (theme-filter bookmarks survive).
    theme_ids = {
        theme["name"]: uuid.uuid5(_THEME_ID_NAMESPACE, f"{task_id}:{theme['name']}")
        for theme in themes
    }
