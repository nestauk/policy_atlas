"""Pure tests for the 017 task plan model and composer."""
...
def _plan(**overrides: Any) -> TaskPlan:
    return TaskPlan.model_validate(_payload(**overrides))
...
def _matrix_plans() -> list[TaskPlan]:
    plans: list[TaskPlan] = []
...
def test_round_trip_payload_composes_to_identical_chain() -> None:
    ...
    round_tripped = TaskPlan.model_validate(plan.model_dump())
