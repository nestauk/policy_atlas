"""Task 024 Task 14 — the agent backend seam + gated watch invocation.
...
def test_stub_is_scriptable_and_counts_calls() -> None:
    ...
    stub = StubAgentBackend(decide_responses=[a, b])
...
def test_stub_satisfies_protocol() -> None:
    backend: AgentBackend = StubAgentBackend()
...
def test_model_routing_by_moment() -> None:
    ...
    assert AGENT_TRIAGE_MODEL != AGENT_MODEL
    assert "mini" in AGENT_TRIAGE_MODEL
...
def _decide(stub: AgentBackend) -> Any:
...
def test_fallback_loop_caps_at_two_then_escalates() -> None:
    ...
    stub = StubAgentBackend(decide_responses=[insufficient])  # repeats
    result = _decide(stub)
...
def test_hook_maps_author_to_apply() -> None:
    ...
    stub = StubAgentBackend(decide_responses=decide)
...
def test_hook_maps_proceed_to_proceed() -> None:
    stub = StubAgentBackend()  # decide → proceed
    outcome = build_watch_discretion_hook(stub)(_ctx())
    assert outcome.interpreted_action == "proceed"
    assert outcome.profile is not None  # the watch decided (attributed to agent)
...
# --------------------------------------------------------------------------
# Layer 2 — DB-backed runner integration (deterministic stub agent)
# --------------------------------------------------------------------------


class _SteerPointAgent:
...
class _PickAuthoredIO:
    ...
    ``Adjust`` carrying ``authored_by='agent'`` (the user decided; the
    agent authored). ``confirm_result`` answers the FIX-C mode+delta gate.
...
def _judgement_events(engine: Engine, task_id: uuid.UUID) -> list[dict[str, Any]]:
...
def _agent_decisions(engine: Engine, task_id: uuid.UUID) -> list[dict[str, Any]]:
    with engine.connect() as conn:
        return [
            entry["payload"]
            for entry in events.read(conn, task_id)
            if entry["event_type"] == steering_events.STEERING_DECISION
            and entry["payload"].get("decided_by") == "agent"
        ]


def _all_decisions(engine: Engine, task_id: uuid.UUID) -> list[dict[str, Any]]:
    with engine.connect() as conn:
        return [
            entry["payload"]
            for entry in events.read(conn, task_id)
...
def _plan_versions(engine: Engine, task_id: uuid.UUID) -> set[int]:
    with engine.connect() as conn:
        return {
            row.version
            for row in conn.execute(
                select(task_plan.c.version).where(
...
def test_clean_boundary_emits_event_and_makes_no_backend_call(
    ...
    try:
        ...
        judged = _judgement_events(engine, task_id)
...
def test_trigger_fired_boundary_triages_not_notable(
    ...
    try:
        ...
        for name in ("p1_coverage_triggers", "grouping_flag_triggers", "steer_point_triggers"):
            monkeypatch.setattr(runner_module, name, lambda *a, **k: [])
        stub = StubAgentBackend()  # triage → not notable
        plan = _base_plan(steering_mode="minimal", steer_point_defaults=[])
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        run_plan(
            engine,
        ...
        judged = _judgement_events(engine, task_id)
        # P2 (evidence_search_coverage) fires in minimal → it pauses → decision point.
...
def test_anomalous_check_in_triages(engine: Engine) -> None:
    ...
    try:
        ...
        stub = StubAgentBackend(triage_responses=notable)
...
def test_attended_pause_carries_authored_options(engine: Engine) -> None:
    ...
    try:
        ...
        orch = _SteerPointAgent(at="deepening_selection", decide=authored)
        io = _CapturingIO()
        ...
        assert p3_pause["authored_by"] == "agent"
...
def test_authored_option_cap_and_endorsement_keep_the_canonical_floor(engine: Engine) -> None:
    ...
    try:
        ...
        orch = _SteerPointAgent(at="deepening_selection", decide=endorsement)
        io = _CapturingIO()
        plan = _base_plan(steering_mode="moderate", steer_point_defaults=[])
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        run_plan(
...
def test_authoring_failure_leaves_canonical_menu_unchanged(engine: Engine) -> None:
    ...
    try:
        ...
        orch = _SteerPointAgent(at="deepening_selection", decide=no_options)
        io = _CapturingIO()
        plan = _base_plan(steering_mode="moderate", steer_point_defaults=[])
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        outcome = run_plan(
...
def test_watch_authored_replacement_applies_only_after_user_pick_and_confirm(
    ...
    The applied decision is decided_by=user, authored_by=agent, confirmed=true,
    and NO steering.decision at the attended pause is decided_by=agent."""
    task_id: uuid.UUID | None = None
    try:
        task_id, scope_id = _seed_task(engine)
        orch = _SteerPointAgent(at="deepening_selection", decide=_authored_reselect())
...
def test_watch_authored_replacement_fails_closed_without_confirm(
    ...
    (authored_by=agent), and no decision is decided_by=agent."""
    task_id: uuid.UUID | None = None
    try:
        task_id, scope_id = _seed_task(engine)
        orch = _SteerPointAgent(at="deepening_selection", decide=_authored_reselect())
        ...
        assert _plan_versions(engine, task_id) == {1}
        decisions = _all_decisions(engine, task_id)
        assert not any(
        ...
        assert any(
            d.get("authored_by") == "agent" and d.get("confirmed") is False
            for d in decisions
        )
        assert not _agent_decisions(engine, task_id)
    finally:
...
def test_attended_floor_pause_not_suppressible_by_not_notable_watch(
    ...
    try:
        ...
        stub = StubAgentBackend()  # default triage → not notable
        io = _CapturingIO()
        plan = _base_plan(steering_mode="moderate", steer_point_defaults=[])
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        outcome = run_plan(
...
def test_backend_error_degrades_to_deterministic_floor(engine: Engine) -> None:
    ...
    try:
        task_id, scope_id = _seed_task(engine)
        plan = _base_plan(steering_mode="moderate", steer_point_defaults=[])
        # Baseline with no agent.
        plan_id_a = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        baseline = run_plan(
            engine,
            task_id=task_id,
            evidence_scope_id=scope_id,
    ...
    try:
        task_id, scope_id = _seed_task(engine)
        plan = _base_plan(steering_mode="moderate", steer_point_defaults=[])
        plan_id_b = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        raised = run_plan(
            engine,
            task_id=task_id,
            evidence_scope_id=scope_id,
...
def test_unattended_decide_applies_via_hook(engine: Engine) -> None:
    ...
    attributed to the agent (reasoning + execution profile carried)."""
    task_id: uuid.UUID | None = None
    try:
        task_id, scope_id = _seed_task(engine)
        ...
        orch = _SteerPointAgent(at="deepening_selection", decide=reselect)
        plan = _base_plan(steering_mode="unattended", steer_point_defaults=[])
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        outcome = run_plan(
            engine,
        ...
        reselect_decision = next(
            p
            for p in _agent_decisions(engine, task_id)
            if p.get("rerun_mode") == "replacement"
        )
        assert reselect_decision["authored_by"] == "agent"
        assert reselect_decision["reasoning"] == "watch deepens on quality"
        assert reselect_decision["execution_profile"]["model"] == AGENT_MODEL
    finally:
        _cleanup_task(engine, task_id)


def test_unattended_hook_not_consulted_when_pinned_rule(engine: Engine) -> None:
    """Authority order (declared rules > agent): a pinned rule wins; the watch
    is never consulted at that point."""
    task_id: uuid.UUID | None = None
    try:
        task_id, scope_id = _seed_task(engine)
        orch = _SteerPointAgent(
        ...
        plan = _base_plan(
            ...
            steer_point_defaults=[
                ...
                {"steer_point": "evidence_search_coverage", "action": "proceed_flag"},
...
def test_fix1_unattended_fired_floor_triages_and_collates_without_pause(
    ...
    try:
        task_id, scope_id = _seed_task(engine)
        _silence_lattice(monkeypatch, keep="after_screen")
        stub = StubAgentBackend()  # triage defaults to not-notable
        io = _RecordingIO()
        plan = _base_plan(steering_mode="unattended", steer_point_defaults=[])
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        outcome = run_plan(
...
def test_fix2_promotion_escalates_anomalous_boundary_to_pause(
    ...
    try:
        ...
        promoted_renders = [r for r in io.renders if "The agent flagged this boundary" in r]
        ...
        assert any(p["verdict"] == "promoted" for p in _judgement_events(engine, task_id))
    finally:
        _cleanup_task(engine, task_id)
