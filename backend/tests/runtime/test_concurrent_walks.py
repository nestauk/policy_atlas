...
introduces concurrent walks (different projects). This is the audit's
prescribed test: two concurrent stub walks on two different projects, driven
from two threads over the same shared engine, must both complete
successfully with fully project-scoped state and no cross-project sequence
collision.
...
project** — concurrent appenders to the *same* project would collide on the
``(project_id, sequence)`` unique constraint (a hard ``IntegrityError``,
...
projects, each with exactly one writer (its own thread here). This test
proves that shape actually works end-to-end — both walks succeed, and
nothing about running them concurrently lets one project's state leak into
the other's.

Uses the walk-scripting idioms from tests/runtime/test_runner.py
(``_base_plan``, ``_seed_project``, ``_runner_backends``) — the same stub
...
def test_two_concurrent_walks_on_different_projects_both_complete(engine: Engine) -> None:
    """Two threads each drive a full stub walk on their own project at the same
    time. Both must succeed, and every durable row each walk wrote must be
    scoped to its own project — no cross-project leakage, no sequence collision."""
    project_a: uuid.UUID | None = None
    project_b: uuid.UUID | None = None
    try:
        project_a, scope_a = _seed_project(engine)
        project_b, scope_b = _seed_project(engine)

        with ThreadPoolExecutor(max_workers=2) as pool:
            future_a = pool.submit(_run_walk, engine, project_a, scope_a)
            future_b = pool.submit(_run_walk, engine, project_b, scope_b)
        ...
        with engine.connect() as conn:
            # --- runs/capability_run read-back is fully project-scoped -----
            rows_a = conn.execute(
                select(runs.c.run_id, runs.c.project_id).where(runs.c.project_id == project_a)
            ).fetchall()
            rows_b = conn.execute(
                select(runs.c.run_id, runs.c.project_id).where(runs.c.project_id == project_b)
            ).fetchall()
            assert {row.run_id for row in rows_a} == run_ids_a
            assert {row.run_id for row in rows_b} == run_ids_b
            assert all(row.project_id == project_a for row in rows_a)
            assert all(row.project_id == project_b for row in rows_b)

            cap_rows_a = conn.execute(
                select(capability_run.c.status, capability_run.c.project_id).where(
                    capability_run.c.project_id == project_a
                )
            ).fetchall()
            cap_rows_b = conn.execute(
                select(capability_run.c.status, capability_run.c.project_id).where(
                    capability_run.c.project_id == project_b
                )
            ).fetchall()
            ...
            # --- event_log is fully project-scoped; no cross-project rows ---
            log_a = events.read(conn, project_a)
            log_b = events.read(conn, project_b)

        assert log_a, "expected event_log rows for project A"
        assert log_b, "expected event_log rows for project B"
        # events.read scopes by project_id at the SQL WHERE, and the composite
        # FK (event_log(run_id, project_id) -> runs(run_id, project_id))
        # enforces this at the DB level; assert the walk's own run ids never
        # appear under the other project's log (belt-and-braces).
        ...
        # --- no cross-project sequence collision: each project's sequence
        # column is a dense, strictly increasing, non-colliding run of its
        # own (1..N) — the single-writer-per-project invariant held under
        # concurrency because the two writers never touched the same project.
        ...
        _cleanup(engine, project_a)
        _cleanup(engine, project_b)
