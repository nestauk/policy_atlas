...
(declared rules > agent), fired triggers riding every decision payload,
...
def _standing_decisions(engine: Engine, task_id: uuid.UUID) -> list[dict[str, Any]]:
...
def test_pinned_rule_applies_reselect_without_pause(engine: Engine) -> None:
    ...
    try:
        ...
        reselect = next(
            payload
            for payload in _standing_decisions(engine, task_id)
...
def test_hard_stop_rule_aborts_and_cannot_be_overridden(engine: Engine) -> None:
    ...
    try:
        ...
        plan = _base_plan(
            steering_mode="unattended",
            steer_point_defaults=[{"steer_point": "evidence_search_coverage", "action": "stop"}],
        )
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        ...
        with engine.connect() as conn:
            ...
            selection_rows = conn.execute(
                select(selection_result.c.run_id).where(selection_result.c.task_id == task_id)
            ).all()
        ...
        assert "evidence_search_coverage" not in would_proceed_calls
    finally:
        _cleanup_task(engine, task_id)
...
def test_discretion_hook_only_consulted_when_no_pinned_rule(engine: Engine) -> None:
    """Authority order (declared rules > agent): the injected hook is
    ...
    try:
        ...
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=ruled)
        outcome = run_plan(
            engine,
            task_id=task_id,
            evidence_scope_id=scope_id,
...
def test_discretion_hook_consulted_without_rules(engine: Engine) -> None:
    ...
    try:
        ...
        assert {"evidence_search_coverage", "deepening_selection", "synthesis_shape"} <= set(
            consulted
        )
    finally:
        _cleanup_task(engine, task_id)
