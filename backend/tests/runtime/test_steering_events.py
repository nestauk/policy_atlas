...
run-id attachment invariant. Agent/router/watch paths are wired by later
...
def _events_of_type(engine: Engine, task_id: uuid.UUID, event_type: str) -> list[dict[str, Any]]:
    with engine.connect() as conn:
        return [
            entry for entry in events.read(conn, task_id) if entry["event_type"] == event_type
        ]
...
def test_walk_opens_threads_and_closes_capability_run(engine: Engine) -> None:
    ...
    try:
        ...
        assert row.capability == "evidence_search"
...
def test_two_walks_in_one_task_get_distinct_capability_runs(engine: Engine) -> None:
    """A second walk in the same task opens a distinct capability_run."""
    task_id: uuid.UUID | None = None
    try:
        task_id, scope_id = _seed_task(engine)
        plan = _base_plan(
...
def test_pause_and_continue_decision_carry_full_base_and_decision_payloads(
    ...
    try:
        ...
        pauses = _events_of_type(engine, task_id, steering_events.STEERING_PAUSE)
        decisions = _events_of_type(engine, task_id, steering_events.STEERING_DECISION)
...
def test_rejected_adjustment_emits_reason_then_continue(engine: Engine) -> None:
    ...
    try:
        ...
        plan = _base_plan(steering_mode="minimal")
        plan_id = _insert_plan_row(engine, task_id=task_id, scope_id=scope_id, plan=plan)
        # appraise has already run by the P2 (evidence_search_coverage) pause and no
        ...
        io = ScriptedIO(
            by_steer_point={
                "evidence_search_coverage": [Adjust(directive_deltas={"appraise": {}})]
            }
        )
        ...
        rejected = _events_of_type(engine, task_id, steering_events.STEERING_REJECTED)
...
def test_skip_emits_component_skipped_with_reason_and_resolved_run(
    ...
    try:
        ...
        skipped = _events_of_type(engine, task_id, steering_events.COMPONENT_SKIPPED)
