...
def test_tasks_create_list_get_archive_and_owner_404(tmp_path: Path) -> None:
...
def test_tasks_reject_page_sizes_over_contract_cap(tmp_path: Path) -> None:
...
def test_task_with_no_run_reports_source_count_as_none(tmp_path: Path) -> None:
...
def test_task_with_a_run_reports_source_count_as_the_snapshot_row_count(
    ...
    with api_client(tmp_path) as (client, owner, _other):
        ...
        zero_sources = client.get(f"/api/v1/tasks/{task_id}", headers=owner)
        assert zero_sources.status_code == 200
        assert zero_sources.json()["source_count"] == 0

        with engine.begin() as conn:
            for _ in range(3):
                seed_source(conn, uuid.UUID(task_id))

        three_sources = client.get(f"/api/v1/tasks/{task_id}", headers=owner)
