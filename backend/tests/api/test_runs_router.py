...
def test_runs_require_approved_plan_and_hide_cross_owner_projects(tmp_path: Path) -> None:
    ...
    with api_client(tmp_path) as (client, owner, other):
        ...
        no_plan = client.post(f"/api/v1/projects/{project_id}/runs", headers=owner, json={})
        assert no_plan.status_code == 400
        assert no_plan.json()["error"]["code"] == "malformed"

        absent = client.post(f"/api/v1/projects/{uuid.uuid4()}/runs", headers=other, json={})
        cross_owner = client.post(f"/api/v1/projects/{project_id}/runs", headers=other, json={})
