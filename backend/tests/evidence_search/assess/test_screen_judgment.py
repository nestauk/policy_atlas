...
def _screened_payloads(conn: Connection, task_id: uuid.UUID) -> list[dict[str, Any]]:
    return [
...
def _run_one_stage1(
    ...
    *,
    ...
    _, tss_id = seed_source(conn, task_id, meta=doc_metadata)
    ...
    return (
        summary,
        _screen_row(conn, task_id, tss_id),
        _screened_payloads(conn, task_id)[0],
        tss_id,
    )
...
def _seed_stage2_candidate(
    ...
    *,
    ...
    _, tss_id = seed_source(conn, task_id, meta=_metadata(key))
    seed_screening_result(conn, task_id, run_id, scope_id, tss_id, status=stage1_status)
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
...
def test_majority_confidence_differs_from_unanimous_same_vote_strength(
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    _, majority = seed_source(conn, task_id, meta=_metadata("majority"))
    _, unanimous = seed_source(conn, task_id, meta=_metadata("unanimous"))
    ...
    majority_row = _screen_row(conn, task_id, majority)
    unanimous_row = _screen_row(conn, task_id, unanimous)
    ...
    payloads = {
        payload["task_source_snapshot_id"]: payload
        for payload in _screened_payloads(conn, task_id)
    }
...
def test_quorum_failure_with_two_or_three_dead_reps_is_retryable_failed(
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    _, two_failed = seed_source(conn, task_id, meta=_metadata("two_failed"))
    _, all_failed = seed_source(conn, task_id, meta=_metadata("all_failed"))
    ...
    for tss_id in (two_failed, all_failed):
        row = _screen_row(conn, task_id, tss_id)
...
def test_title_only_not_relevant_requires_unanimity_to_exclude(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    _, mixed = seed_source(conn, task_id, meta=_metadata("mixed", abstract=None))
    _, unanimous = seed_source(conn, task_id, meta=_metadata("unanimous", abstract=None))
    _, unsure_dissent = seed_source(
        conn, task_id, meta=_metadata("unsure_dissent", abstract=None)
    )
    ...
    mixed_row = _screen_row(conn, task_id, mixed)
    unanimous_row = _screen_row(conn, task_id, unanimous)
    unsure_dissent_row = _screen_row(conn, task_id, unsure_dissent)
    payloads = {
        payload["task_source_snapshot_id"]: payload
        for payload in _screened_payloads(conn, task_id)
    }
...
def test_stage2_demotes_fulltext_doc_and_effective_row_flips(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = _seed_stage2_candidate(conn, task_id, run_id, scope_id)
    stage2_run_id = seed_run(conn, task_id)
    ...
    stage2_row = _screen_row(conn, task_id, tss_id, stage=2)
    effective = _effective_row(conn, task_id, scope_id, tss_id)
...
def test_stage2_unsure_confirms_relevant_at_half_with_flag(conn: Connection) -> None:
    ...
    row = _screen_row(conn, task_id, tss_id, stage=2)
    payload = _screened_payloads(conn, task_id)[0]
...
def test_stage2_skips_abstract_only_doc_without_row(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    snap_id, tss_id = seed_source(conn, task_id, meta=_metadata("doc"))
    conn.execute(
    ...
    assert _stage_row_count(conn, task_id, tss_id, stage=2) == 0


def test_stage2_no_rescue_invariant_blocks_stage1_excludes(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = _seed_stage2_candidate(
        conn,
        task_id,
        run_id,
        scope_id,
    ...
    with pytest.raises(RuntimeError, match="requires a relevant stage-1 row"):
        _assert_stage1_relevant(conn, scope_id=scope_id, tss_id=tss_id, generation=0)
...
def test_paired_injection_fixture_persists_identical_decisions_and_prompt_data(
    ...
    clean_row = _screen_row(conn, task_id, clean)
    adversarial_row = _screen_row(conn, task_id, adversarial)
