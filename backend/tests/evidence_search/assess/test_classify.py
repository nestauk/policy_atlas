...
def test_classify_sources_round_trip(conn: Connection) -> None:
    ...
    classify_sources(conn, task_id=pid, run_id=rid, context=ctx)

    rows = conn.execute(
        select(source_classification_result).where(
            source_classification_result.c.task_id == pid
        )
    ).fetchall()
    assert len(rows) == 1
    assert rows[0].evidence_scope_id == scope_id
    assert rows[0].task_source_snapshot_id == tss_id
...
def test_classify_sources_fan_out_by_type_matches_sentinels(conn: Connection) -> None:
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_rct = seed_source(conn, pid, meta={"_stub_rct": True})
    _, tss_policy = seed_source(conn, pid, meta={"_stub_policy_guidance": True})
    _, tss_qual = seed_source(conn, pid, meta={"_stub_qualitative": True})
    _, tss_unknown = seed_source(conn, pid)
    for tss_id in (tss_rct, tss_policy, tss_qual, tss_unknown):
        seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant")

    ctx = ClassifyContext(scope_id=scope_id, intent="Test", context={})
    counts = classify_sources(conn, task_id=pid, run_id=rid, context=ctx)
    ...
    by_tss = {r.task_source_snapshot_id: r.primary_evidence_type for r in rows}
    assert by_tss[tss_rct] == "RCTs and Quasi-Experimental Studies"
    assert by_tss[tss_policy] == "Policy Syntheses & Guidance Documents"
    assert by_tss[tss_qual] == "Qualitative & Contextual Evidence"
    assert by_tss[tss_unknown] == "Unknown / Insufficient information"
...
def test_classify_sources_non_evidence_persists(conn: Connection) -> None:
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_id = seed_source(conn, pid, meta={"_stub_non_evidence": True})
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant")
...
def test_classify_sources_skips_not_relevant(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_relevant = seed_source(conn, pid)
    _, tss_not_relevant = seed_source(conn, pid)
    seed_screening_result(conn, pid, rid, scope_id, tss_relevant, status="relevant")
...
def test_classify_sources_uses_effective_screen_rows(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_demoted = seed_source(conn, pid, meta={"_stub_rct": True})
    _, tss_confirmed = seed_source(conn, pid, meta={"_stub_policy_guidance": True})
    seed_screening_result(conn, pid, rid, scope_id, tss_demoted, status="relevant")
    seed_screening_result(conn, pid, rid, scope_id, tss_confirmed, status="relevant")
    conn.execute(source_screening_result.insert().values(
        source_screening_result_id=uuid.uuid4(),
        evidence_scope_id=scope_id,
        task_source_snapshot_id=tss_demoted,
        task_id=pid,
        screened_by_run_id=rid,
        status="not_relevant",
    ...
    conn.execute(source_screening_result.insert().values(
        source_screening_result_id=uuid.uuid4(),
        evidence_scope_id=scope_id,
        task_source_snapshot_id=tss_confirmed,
        task_id=pid,
        screened_by_run_id=rid,
        status="relevant",
        screen_basis="title_abstract",
    ...
    assert len(rows) == 1
    assert rows[0].task_source_snapshot_id == tss_confirmed
...
def test_excluded_retracted_never_reaches_classify_eligibility(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, pid)
    _, tss_relevant = seed_source(conn, pid)
    _, tss_retracted = seed_source(conn, pid)
    seed_screening_result(conn, pid, rid, scope_id, tss_relevant, status="relevant")
    seed_screening_result(
        conn, pid, rid, scope_id, tss_retracted, status="excluded_retracted"
    )
    ...
    assert [row.task_source_snapshot_id for row in rows] == [tss_relevant]
...
def test_classify_sources_idempotent_rerun(conn: Connection) -> None:
    ...
    ctx = ClassifyContext(scope_id=scope_id, intent="Test", context={})
    first = classify_sources(conn, task_id=pid, run_id=rid, context=ctx)
    assert first["classified"] == 1
    assert first["already_classified"] == 0

    second = classify_sources(conn, task_id=pid, run_id=rid, context=ctx)
...
def test_cross_task_fk_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError):
        conn.execute(sa.text(
            ...
            "(source_classification_result_id, evidence_scope_id, task_source_snapshot_id, "
            " task_id, classified_by_run_id, primary_evidence_type, classified_at) "
            "VALUES (:scrid, :scope_id, :tss_id, :pid_a, :rid_a, "
...
def test_classify_sources_doc_exception_isolated(conn: Connection) -> None:
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_boom = seed_source(conn, pid, meta={"_stub_classify_failed": True})
    _, tss_fine = seed_source(conn, pid, meta={"_stub_rct": True})
    seed_screening_result(conn, pid, rid, scope_id, tss_boom, status="relevant")
    seed_screening_result(conn, pid, rid, scope_id, tss_fine, status="relevant")
    ...
    by_tss = {r.task_source_snapshot_id: r.primary_evidence_type for r in rows}
    assert tss_boom not in by_tss
    assert by_tss[tss_fine] == "RCTs and Quasi-Experimental Studies"
    classified_events = [
        e for e in events.read(conn, pid) if e["event_type"] == "source.classified"
    ]
    assert [e["payload"]["task_source_snapshot_id"] for e in classified_events] == [
        str(tss_fine)
    ]

    retry_run = seed_run(conn, pid)
    retry_counts = classify_sources(conn, task_id=pid, run_id=retry_run, context=ctx)
...
def test_harness_classify_component(conn: Connection) -> None:
    ...
    screen_sources(conn, task_id=pid, run_id=rid_screen, context=screen_ctx)

    # Create a second run for classify
    rid_classify = uuid.uuid4()
    conn.execute(runs.insert().values(
        run_id=rid_classify, task_id=pid, status="running", started_at=now()
    ))
...
def test_source_classified_event_payload(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    snap_id, tss_id = seed_source(conn, pid)
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant")
    ...
    assert p["task_source_snapshot_id"] == str(tss_id)
...
def test_classify_sources_writes_bounded_methodological_tags(conn: Connection) -> None:
    ...
    rows = conn.execute(
        ...
        .where(source_tag.c.task_source_snapshot_id == tss_id)
...
def test_delete_task_data_removes_classification(conn: Connection) -> None:
