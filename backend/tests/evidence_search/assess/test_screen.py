...
def test_tss_has_composite_unique(conn: Connection) -> None:
    ...
    names = {uc["name"] for uc in inspector.get_unique_constraints("task_source_snapshot")}
    assert "uq_tss_id_task" in names


def test_evidence_scope_columns(conn: Connection) -> None:
    inspector = inspect(conn)
    cols = {c["name"] for c in inspector.get_columns("evidence_scope")}
    assert {"context", "intent", "task_id", "evidence_scope_id", "created_at"} <= cols
...
def test_ssr_columns(conn: Connection) -> None:
    ...
    assert {
        "source_screening_result_id", "evidence_scope_id", "task_source_snapshot_id",
        "task_id", "screened_by_run_id", "status", "screen_basis",
...
def _ssr_insert(conn: Connection, task_id: uuid.UUID, run_id: uuid.UUID,
                scope_id: uuid.UUID, tss_id: uuid.UUID, **overrides: object) -> None:
...
def test_ck_bad_status(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError):
        _ssr_insert(conn, pid, rid, scope_id, tss_id, status="unknown_status")
    conn.rollback()
    conn.begin()
...
def test_ck_bad_basis(conn: Connection) -> None:
    ...
    _, tss_id = seed_source(conn, pid)
    with pytest.raises(IntegrityError):
        _ssr_insert(conn, pid, rid, scope_id, tss_id, screen_basis="body_text")
    conn.rollback()
    conn.begin()
...
def test_ck_confidence_out_of_range(conn: Connection) -> None:
    ...
    _, tss_id = seed_source(conn, pid)
    with pytest.raises(IntegrityError):
        _ssr_insert(conn, pid, rid, scope_id, tss_id, screen_decision_confidence=1.5)
    conn.rollback()
    conn.begin()
...
def test_ck_failed_with_non_null_basis(conn: Connection) -> None:
    ...
    _, tss_id = seed_source(conn, pid)
    with pytest.raises(IntegrityError):
        _ssr_insert(conn, pid, rid, scope_id, tss_id,
...
def test_partial_unique_stage_matrix(conn: Connection) -> None:
    ...
    _ssr_insert(conn, pid, rid, scope_id, tss_id, screen_stage=1)
    _ssr_insert(conn, pid, rid, scope_id, tss_id, screen_stage=2, screen_basis="full_text")

    with pytest.raises(IntegrityError, match="uq_ssr_scope_source_stage"), conn.begin_nested():
        _ssr_insert(conn, pid, rid, scope_id, tss_id, screen_stage=2, screen_basis="full_text")

    _ssr_insert(
        conn, pid, rid, scope_id, tss_id, screen_stage=1,
        status="failed", screen_basis=None, screen_decision_confidence=None,
    )
    _ssr_insert(
        conn, pid, rid, scope_id, tss_id, screen_stage=2,
        status="failed", screen_basis=None, screen_decision_confidence=None,
    )
    _ssr_insert(
        conn, pid, rid, scope_id, tss_id, screen_stage=2,
...
def test_screen_sources_relevant_with_abstract(conn: Connection) -> None:
    ...
    screen_sources(conn, task_id=pid, run_id=rid, context=ctx)

    row = conn.execute(
        select(source_screening_result).where(source_screening_result.c.task_id == pid)
    ).one()
...
def test_screen_sources_mixed_counts(conn: Connection) -> None:
    ...
    counts = screen_sources(conn, task_id=pid, run_id=rid, context=ctx)
...
def test_screen_sources_retry_after_failure_preserves_failed_rows_and_adds_new(
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    snap_id, tss_id = seed_source(conn, pid, meta={"_stub_failed": True})
    ctx = ScreenContext(scope_id=scope_id, intent="Test", context={})

    first = screen_sources(conn, task_id=pid, run_id=rid, context=ctx)
    assert first["screened"] == 1
    assert first["failed"] == 1

    second = screen_sources(conn, task_id=pid, run_id=seed_run(conn, pid), context=ctx)
    ...
    third = screen_sources(conn, task_id=pid, run_id=seed_run(conn, pid), context=ctx)
...
def test_screen_sources_idempotent_rerun(conn: Connection) -> None:
    """Re-running screen_sources for the same task does not raise or duplicate rows."""
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    ...
    second = screen_sources(conn, task_id=pid, run_id=rid, context=ctx)
...
def test_source_screened_event_payload(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    snap_id, tss_id = seed_source(conn, pid, meta={"abstract": "Policy doc."})
    ctx = ScreenContext(scope_id=scope_id, intent="Test", context={})
    ...
    assert payload["task_source_snapshot_id"] == str(tss_id)
...
def test_unique_constraint_scope_source(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_id = seed_source(conn, pid, meta={"abstract": "X"})
    ctx = ScreenContext(scope_id=scope_id, intent="Test", context={})
...
def test_cross_task_fk_rejected(conn: Connection) -> None:
    pid_a, rid_a = seed_task_and_run(conn)
    pid_b, _ = seed_task_and_run(conn)

    scope_id = seed_scope(conn, pid_a)
    _, tss_id_b = seed_source(conn, pid_b)

    # scope belongs to task A, tss belongs to task B → FK violation
    _cross_cols = (
        "source_screening_result_id, evidence_scope_id, task_source_snapshot_id, "
        "task_id, screened_by_run_id, status, screen_basis, "
    ...
    with pytest.raises(IntegrityError):
        conn.execute(sa.text(
            ...
            "VALUES (:ssrid, :scope_id, :tss_id, :pid_a, :rid_a, "
...
class _CallCountingBackend:
    """Delegates to the deterministic stub but records which tss_id every
    ...
    def __init__(self) -> None:
        self._stub = StubScreeningBackend()
        self.envelope_tss_ids: list[str] = []
    ...
    def screen_envelope(
        ...
        *,
        ...
        self.envelope_tss_ids.append(payload.tss_id)
        return self._stub.screen_envelope(payload, rep_index=rep_index)
...
def _seed_retracted_source(conn: Connection, task_id: uuid.UUID) -> uuid.UUID:
    _, tss_id = seed_source(
        conn,
        task_id,
...
def test_screen_sources_excludes_retracted_doc_without_backend_call(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    retracted_tss = _seed_retracted_source(conn, pid)
    _, clean_tss = seed_source(conn, pid, meta={"abstract": "Clean policy text."})
    ctx = ScreenContext(scope_id=scope_id, intent="Test", context={})
    backend = _CallCountingBackend()

    counts = screen_sources(
        conn, task_id=pid, run_id=rid, context=ctx, screening_backend=backend
    )

    assert str(retracted_tss) not in backend.envelope_tss_ids
    assert str(clean_tss) in backend.envelope_tss_ids

    retracted_row = conn.execute(
        select(source_screening_result)
        .where(source_screening_result.c.task_source_snapshot_id == retracted_tss)
    ).one()
    ...
    clean_row = conn.execute(
        select(source_screening_result)
        .where(source_screening_result.c.task_source_snapshot_id == clean_tss)
    ).one()
...
def test_excluded_retracted_effective_row_has_no_stage2_row(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, pid)
    retracted_tss = _seed_retracted_source(conn, pid)
    seed_ingested_full_text(conn, tss_id=retracted_tss, chunks=["Full text, if it mattered."])
    ctx1 = ScreenContext(scope_id=scope_id, intent="Test", context={})
    screen_sources(conn, task_id=pid, run_id=rid, context=ctx1)

    ctx2 = ScreenContext(
        scope_id=scope_id, intent="Test", context={"screening": {"stage": 2}}
    )
    stage2_counts = screen_sources(
        conn, task_id=pid, run_id=seed_run(conn, pid), context=ctx2
    )
...
def _seed_stage2_candidate(
    ...
    Returns (tss_id, chunk_snapshot_id).
    """
    _, tss_id = seed_source(conn, pid, meta={"title": "Stage-2 candidate"})
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant", screen_stage=1)
    chunk_snapshot_id = seed_ingested_full_text(conn, tss_id=tss_id, chunks=chunks)
    return tss_id, chunk_snapshot_id
...
def test_stage2_prefix_hydration_equivalence_large_doc(conn: Connection) -> None:
    ...
    tss_id, chunk_snapshot_id = _seed_stage2_candidate(conn, pid, rid, scope_id, chunk_texts)
    ...
    docs, skipped = _load_stage2_docs(conn, task_id=pid, scope_id=scope_id)
    assert skipped == 0
    assert len(docs) == 1
    assert docs[0].tss_id == tss_id
...
def test_stage2_prefix_hydration_only_loads_the_prefix(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, pid)
    chunk_texts = [f"chunk{i:03d}".ljust(3000, "x") for i in range(30)]
    _tss_id, chunk_snapshot_id = _seed_stage2_candidate(conn, pid, rid, scope_id, chunk_texts)
    ...
    docs, _skipped = _load_stage2_docs(conn, task_id=pid, scope_id=scope_id)
...
def test_stage2_prefix_hydration_oversize_single_chunk(conn: Connection) -> None:
    ...
    _tss_id, chunk_snapshot_id = _seed_stage2_candidate(
...
def test_screen_criteria_compose_into_stage1_intent(conn: Connection) -> None:
    ...
    screen_sources(conn, task_id=pid, run_id=rid, context=ctx, screening_backend=backend)
...
def test_screen_criteria_compose_into_stage2_intent(conn: Connection) -> None:
    ...
    _, tss_id = seed_source(conn, pid, meta={"title": "Stage-2 candidate"})
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant", screen_stage=1)
    seed_ingested_full_text(conn, tss_id=tss_id, chunks=["Some full-text content."])
    ctx = ScreenContext(
        scope_id=scope_id,
        intent="Housing policy scope intent.",
...
def test_screen_criteria_leave_evidence_scope_intent_unchanged(conn: Connection) -> None:
    ...
    plan = Plan(component="screen", evidence_scope_id=scope_id)
    config = compile(plan)
    run_harness(conn, config=config, task_id=pid, run_id=rid, provider=StubEchoProvider())
...
def test_criteria_key_handling_confined_to_screen_module() -> None:
    ...
    consumer_modules = [
        ...
        "evidence_search/sourcing/search_live.py",
        "evidence_search/sourcing/search_loop.py",
        "evidence_search/sourcing/search_prompts.py",
        "evidence_search/synthesis/synthesis_backend.py",
        "evidence_search/synthesis/synthesis_tools.py",
        "evidence_search/synthesis/synthesise.py",
    ]
...
def _effective_row(
    conn: Connection, pid: uuid.UUID, scope_id: uuid.UUID, tss_id: uuid.UUID
) -> Any:
    effective = effective_screen_rows()
    return conn.execute(
        select(effective)
        .where(effective.c.task_id == pid)
        .where(effective.c.evidence_scope_id == scope_id)
        .where(effective.c.task_source_snapshot_id == tss_id)
    ).one()


def _rows_for(conn: Connection, tss_id: uuid.UUID) -> list[dict[str, Any]]:
...
def test_effective_row_newest_generation_supersedes_stale_stage2(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, pid)
    _, tss_id = seed_source(conn, pid, meta={"abstract": "x"})
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant",
                          screen_stage=1, screen_generation=0)
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant",
                          screen_stage=2, screen_basis="full_text", screen_generation=0)
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="not_relevant",
                          screen_stage=1, screen_generation=1)

    row = _effective_row(conn, pid, scope_id, tss_id)
...
def test_failed_rows_never_effective_at_any_generation(conn: Connection) -> None:
    ...
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant",
                          screen_stage=1, screen_generation=0)
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="failed",
                          screen_stage=1, screen_generation=1)
...
def test_rescreen_writes_next_generation_bypasses_skip_and_leaves_old_rows_identical(
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_id = seed_source(conn, pid, meta={"abstract": "Policy text."})

    first = screen_sources(
        conn, task_id=pid, run_id=rid,
    ...
    rows_after = _rows_for(conn, tss_id)
    ...
    assert _effective_row(conn, pid, scope_id, tss_id).screen_generation == 1
...
def test_absent_rescreen_matches_as_built_skip_and_generation_zero(conn: Connection) -> None:
    ...
    assert first["screen_generation"] == 0
    assert first["rescreen"] is False
    assert [r["screen_generation"] for r in _rows_for(conn, tss_id)] == [0]

    second = screen_sources(conn, task_id=pid, run_id=seed_run(conn, pid), context=ctx)
    assert second["screened"] == 0
    assert len(_rows_for(conn, tss_id)) == 1
...
def test_stage2_inherits_stage1_generation_and_demote_only_within_generation(
    ...
    seed_screening_result(conn, pid, rid, scope_id, confirm, status="relevant",
                          screen_stage=1, screen_generation=1)
    seed_ingested_full_text(conn, tss_id=confirm, chunks=["confirm full text evidence."])
    ...
    seed_screening_result(conn, pid, rid, scope_id, demote, status="relevant",
                          screen_stage=1, screen_generation=1)
    seed_ingested_full_text(conn, tss_id=demote, chunks=["demote full text evidence."])
...
def test_stage2_rescreen_same_generation_collision_is_loud_and_reason_coded(
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_id = seed_source(conn, pid, meta={"title": "collide"})
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant",
                          screen_stage=1, screen_generation=1)
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant",
                          screen_stage=2, screen_basis="full_text", screen_generation=1)
    seed_ingested_full_text(conn, tss_id=tss_id, chunks=["full text evidence."])

    with pytest.raises(ScreenSupersessionError) as excinfo:
        screen_sources(
            conn, task_id=pid, run_id=seed_run(conn, pid),
            context=ScreenContext(
                scope_id=scope_id, intent="Test",
...
def test_consumer_characterise_screened_sources_follows_new_generation(
    ...
    sources = screened_sources(conn, task_id=pid, scope_id=scope_id)
    ids = {source.tss_id for source in sources}
...
def test_consumer_characterise_base_counts_follows_new_generation(
    ...
    counts = _base_counts(conn, task_id=pid, scope_id=scope_id)
...
def test_consumer_stage2_skip_advances_doc_at_new_generation(conn: Connection) -> None:
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_id = seed_source(conn, pid, meta={"title": "advance"})
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant",
                          screen_stage=1, screen_generation=0)
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="relevant",
                          screen_stage=2, screen_basis="full_text", screen_generation=0)
    ...
    docs, skipped = _load_stage2_docs(conn, task_id=pid, scope_id=scope_id)
    assert skipped == 0
    assert [doc.tss_id for doc in docs] == [tss_id]
