...
def _classification_row(
    ...
    return conn.execute(
        ...
        .where(source_classification_result.c.task_source_snapshot_id == tss_id)
    ).one()


def _classification_rows(conn: Connection, task_id: uuid.UUID) -> list[Any]:
    return list(
        conn.execute(
            select(source_classification_result)
            .where(source_classification_result.c.task_id == task_id)
            .order_by(source_classification_result.c.task_source_snapshot_id)
        ).fetchall()
    )


def _classified_payloads(conn: Connection, task_id: uuid.UUID) -> list[dict[str, Any]]:
    return [
...
def test_unknown_boundary_persists_and_other_persists_for_news_item(
    ...
    assert (
        _classification_row(conn, task_id, evidence_like).primary_evidence_type
        == UNKNOWN_TYPE
    )
    assert _classification_row(conn, task_id, news_item).primary_evidence_type == OTHER_TYPE
...
def test_classified_event_carries_confidence_reason_but_table_does_not(
    ...
    payload = _classified_payloads(conn, task_id)[0]
...
def test_classify_sources_passes_source_tags_as_label_priors(conn: Connection) -> None:
    ...
    priors_by_tss = {payload.tss_id: payload.priors for payload in backend.payloads}
    assert priors_by_tss[str(tss_a)]["label_priors"] == [
        {"tag": "housing", "tag_type": TOPIC_THEME, "asserted_by": "openalex"}
    ]
    assert priors_by_tss[str(tss_b)]["label_priors"] == [
        {
            "tag": "randomised trial",
            "tag_type": METHODOLOGICAL_STRUCTURAL,
            "asserted_by": "overton_llm",
...
def test_classify_failure_writes_no_row_no_event_and_next_run_retries(
    ...
    assert _classification_rows(conn, task_id) == []
    assert _classified_payloads(conn, task_id) == []
    ...
    assert _classification_row(conn, task_id, tss_id).primary_evidence_type == RCT_TYPE
    assert len(_classified_payloads(conn, task_id)) == 1
...
def test_paired_classify_injection_fixture_is_inert_data(
    ...
    assert _classification_row(conn, task_id, clean).primary_evidence_type == POLICY_TYPE
    assert (
        _classification_row(conn, task_id, adversarial).primary_evidence_type
        == POLICY_TYPE
    )
