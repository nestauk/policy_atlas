...
from policy_atlas.evidence_search.corpus import select as select_corpus
...
def _seed_classified(
    ...
    Returns (source_snapshot_id, tss_id).
    """
...
def _appraisal_rows(conn: Connection, task_id: uuid.UUID) -> list[sa.Row[Any]]:
...
def test_appraise_sources_round_trip_via_classify(conn: Connection) -> None:
    ...
    for sentinel, score in sentinels.items():
        _, tss_id = seed_source(conn, pid, meta={sentinel: True})
        seed_screening_result(conn, pid, rid, scope_id, tss_id)
        tss_by_score[tss_id] = score
    classify_sources(
        conn, task_id=pid, run_id=rid,
        context=ClassifyContext(scope_id=scope_id, intent="Test", context={}),
    )

    counts = appraise_sources(conn, task_id=pid, run_id=rid, context=_ctx(scope_id))

    assert counts["appraised"] == 3
    assert counts["by_score"] == {5: 1, 4: 1, 1: 1}
    rows = _appraisal_rows(conn, pid)
    assert {r.task_source_snapshot_id: r.quality_score for r in rows} == tss_by_score
...
def test_demoted_doc_not_counted_unclassified(conn: Connection) -> None:
    ...
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="not_relevant", screen_stage=2)
...
def test_classified_then_demoted_not_appraised_on_rerun(conn: Connection) -> None:
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_id = _seed_classified(conn, pid, rid, scope_id, "Systematic Review and Meta-Analysis")
    seed_screening_result(conn, pid, rid, scope_id, tss_id, status="not_relevant", screen_stage=2)
    ...
    # never twice.
    _, tss_second = _seed_classified(conn, pid, rid, scope_id, "Observational Research Studies")
    counts = appraise_sources(conn, task_id=pid, run_id=rid, context=_ctx(scope_id))
    assert counts["appraised"] == 1
    seed_screening_result(
        conn, pid, rid, scope_id, tss_second, status="not_relevant", screen_stage=2
    )
    counts = appraise_sources(conn, task_id=pid, run_id=rid, context=_ctx(scope_id))
...
def test_appraise_sources_idempotent_rerun(conn: Connection) -> None:
    ...
    first = appraise_sources(conn, task_id=pid, run_id=rid, context=_ctx(scope_id))
    assert first["appraised"] == 2
    assert first["already_appraised"] == 0

    second = appraise_sources(conn, task_id=pid, run_id=rid, context=_ctx(scope_id))
...
def test_mixed_rerun_skip_counts_stable_and_invariant_holds(conn: Connection) -> None:
    ...
    _, tss_unclassified = seed_source(conn, pid)
    seed_screening_result(conn, pid, rid, scope_id, tss_unclassified)
...
def test_rubric_version_persisted_on_every_row(conn: Connection) -> None:
    ...
    appraise_sources(conn, task_id=pid, run_id=rid, context=_ctx(scope_id))
...
def test_overridden_rubric_changes_persisted_scores_and_version(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_low = _seed_classified(conn, pid, rid, scope_id, "Expert Opinion and Commentary")

    override = {"Expert Opinion and Commentary": 5}
    context = AppraiseContext(
        scope_id=scope_id, intent="Test", context={"appraisal": {"rubric": override}}
    )
    counts = appraise_sources(conn, task_id=pid, run_id=rid, context=context)

    assert counts["appraised"] == 1
    (row,) = _appraisal_rows(conn, pid)
    assert row.task_source_snapshot_id == tss_low
...
def test_overridden_rubric_version_travels_in_event_payload(conn: Connection) -> None:
    ...
    appraise_sources(conn, task_id=pid, run_id=rid, context=context)
...
def test_downstream_coherence_select_consumes_overridden_scores_unchanged(
    ...
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_a = _seed_classified(conn, pid, rid, scope_id, "Systematic Review and Meta-Analysis")
    _, tss_b = _seed_classified(conn, pid, rid, scope_id, "Expert Opinion and Commentary")
    ...
    persisted = {r.task_source_snapshot_id: r.quality_score for r in _appraisal_rows(conn, pid)}
    assert persisted[tss_a] == 1
    assert persisted[tss_b] == 5
    ...
    sources = {s.tss_id: s for s in screened_sources(conn, task_id=pid, scope_id=scope_id)}
    assert sources[tss_a].quality_score == 1
    assert sources[tss_b].quality_score == 5
    ...
    stratum = select_corpus.SelectionStratum(name="S", candidate_ids=(tss_a, tss_b))
    candidates = [
        select_corpus.SelectionCandidate(source=sources[tss_a], tags=()),
        select_corpus.SelectionCandidate(source=sources[tss_b], tags=()),
    ]
    ...
    assert [record["tss_id"] for record in outcome.selected] == [str(tss_b)]
...
def test_cross_task_fk_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError):
        conn.execute(sa.text(
            ...
            "(source_appraisal_result_id, evidence_scope_id, task_source_snapshot_id, "
            " task_id, appraised_by_run_id, quality_score, rubric_version, appraised_at) "
            "VALUES (:sarid, :scope_id, :tss_id, :pid_a, :rid_a, 3, 'v2-hierarchy-v1', :ts)"
        ), {
            "sarid": uuid.uuid4(),
            "scope_id": scope_id,
            "tss_id": tss_id_b,
...
def test_uq_scope_source_duplicate(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError):
        conn.execute(source_appraisal_result.insert().values(
            source_appraisal_result_id=uuid.uuid4(),
            evidence_scope_id=scope_id,
            task_source_snapshot_id=row.task_source_snapshot_id,
            task_id=pid,
            appraised_by_run_id=rid,
            quality_score=5,
            rubric_version=DEFAULT_RUBRIC_VERSION,
...
def test_harness_appraise_component(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, pid)
    _, tss_sr = seed_source(conn, pid, meta={"_stub_systematic_review": True})
    _, tss_non_ev = seed_source(conn, pid, meta={"_stub_non_evidence": True})
    seed_screening_result(conn, pid, rid_screen, scope_id, tss_sr)
    seed_screening_result(conn, pid, rid_screen, scope_id, tss_non_ev)
    classify_sources(
        conn, task_id=pid, run_id=rid_screen,
        context=ClassifyContext(scope_id=scope_id, intent="Test", context={}),
    )

    rid_appraise = uuid.uuid4()
    conn.execute(runs.insert().values(
        run_id=rid_appraise, task_id=pid, status="running", started_at=now()
    ))

    plan = Plan(component="appraise", evidence_scope_id=scope_id)
    outcome = run_harness(
        conn, config=compile(plan), task_id=pid, run_id=rid_appraise,
    ...
    rows = _appraisal_rows(conn, pid)
    assert len(rows) == 1
    assert rows[0].task_source_snapshot_id == tss_sr
    ...
    appraised_events = [e for e in log_entries if e["event_type"] == "source.appraised"]
    assert len(appraised_events) == 1
    assert appraised_events[0]["payload"]["task_source_snapshot_id"] == str(tss_sr)
...
def test_source_appraised_event_payload(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    snap_id, tss_id = _seed_classified(
...
def test_delete_task_data_removes_appraisal(conn: Connection) -> None:
