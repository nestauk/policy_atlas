...
def _strip_abstract(conn: Connection, tss_id: uuid.UUID) -> None:
...
def _selected_records_by_id(row: Any) -> dict[str, dict[str, Any]]:
    return {
        cast("str", record["tss_id"]): cast("dict[str, Any]", record)
        for record in row._mapping["selected"]
    }


def _selected_ids(row: Any) -> set[str]:
    return {record["tss_id"] for record in row._mapping["selected"]}
...
def test_contested_strata_only_and_call_accounting(conn: Connection) -> None:
    ...
    contested_rankable = {str(tss_id) for tss_id in contested if tss_id != must_include}
    ...
    assert called_ids.isdisjoint({str(tss_id) for tss_id in whole})
    ...
    pid2, characterise_run_id2 = seed_task_and_run(conn)
...
def test_scored_docs_sort_before_fallback_docs_with_composite_tie_breaks(
    ...
    selected = [record["tss_id"] for record in row._mapping["selected"]]
    ...
    assert all("composite" in selected_by_id[tss_id] for tss_id in expected)
...
def test_misbehaving_ranker_falls_back_per_doc_without_dropping_candidates(
    ...
    for tss_id in (out_of_range, bool_score, conflict, missing):
        record = selected_by_id[str(tss_id)]
        assert record["rank_fallback"] is True
        assert "llm_score" not in record
...
def test_rerank_reasons_are_inert_data_and_invalid_reasons_fall_back(
    ...
    for tss_id in (control_doc, long_doc):
        record = selected_by_id[str(tss_id)]
        assert record["rank_fallback"] is True
        assert "llm_score" not in record
