...
from policy_atlas.evidence_search.corpus.ranking import RERANK_MODEL, RankedDoc, StubRankingBackend
...
def _selection_row_count(conn: Connection, task_id: uuid.UUID) -> int:
    return int(
        conn.execute(
...
def _docs(
    ...
    return [
        seed_select_doc(conn, task_id, run_id, scope_id, title=f"{prefix}-{index}")
...
def _seed_two_stage_doc(
    ...
    *,
    ...
    _, tss_id = seed_source(
        conn, task_id, meta={"title": title, "abstract": f"Abstract for {title}.", "year": 2026}
    )
...
def test_screened_sources_effective_grain_four_shapes(conn: Connection) -> None:
    ...
    records = {record["tss_id"]: record for record in row._mapping["selected"]}
...
def test_allocation_math_matches_hand_computed_fixture(conn: Connection) -> None:
    pid, characterise_run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
...
def test_must_include_bypasses_budget_and_conflict_is_notable(conn: Connection) -> None:
    ...
    assert row._mapping["excluded"]["notable"] == [
        {"tss_id": str(missing_id), "flag": "must_include_not_in_scope"}
    ]
    ...
    assert a_records[0]["tss_id"] == str(theme_a[0])
...
def test_directive_validation_empty_equivalence_and_boost_reordering(
    ...
    assert row_default._mapping["selected"][0]["tss_id"] == str(uploaded)
    assert row_boost._mapping["selected"][0]["tss_id"] == str(acquired)
    ...
    assert {record["tss_id"] for record in row_all._mapping["selected"]} == {
        str(uploaded),
        str(acquired),
    }
...
def test_missing_characterisation_empty_scope_and_unclustered_select_all(
    ...
    pid, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
...
def test_llm_rerank_contested_scope_and_fallback_ordering(conn: Connection) -> None:
    ...
    assert set(backend.calls[0]) == {str(tss_id) for tss_id in theme_a}
    assert set(backend.calls[0]).isdisjoint({str(tss_id) for tss_id in theme_b})
    ...
    assert selected_a[0]["tss_id"] == backend.scored_id
...
def test_schema_constraints_reject_bad_rows(conn: Connection) -> None:
    ...
    # Cross-task guard: evidence_scope_id belongs to task pid, but run_id
    # belongs to a different task (pid_b) — no (run_id, task_id=pid) row
    # exists in runs, so fk_selr_run_task rejects it.
    pid_b, run_b = seed_task_and_run(conn)
...
def test_text_basis_soft_tilt_ranks_full_text_above_but_never_excludes(
    ...
    assert row._mapping["selected"][0]["tss_id"] == str(full_text_doc)
...
def test_missing_signals_flag_not_block() -> None:
    ...
    stratum = SelectionStratum(name="A", candidate_ids=(tss_id,))
    ...
    assert record["tss_id"] == str(tss_id)
...
def test_directive_tag_boost_reorders_stratum_and_never_excludes(conn: Connection) -> None:
    ...
    insert_source_tags(
        conn, task_id=pid, run_id=characterise_run_id, now=now(),
    ...
    assert row_no_boost._mapping["selected"][0]["tss_id"] == str(better)
    ...
    assert row_boost._mapping["selected"][0]["tss_id"] == str(worse)
...
def test_strata_scope_only_excludes_other_strata_from_ranking(conn: Connection) -> None:
    ...
    selected_tss_ids = {record["tss_id"] for record in row._mapping["selected"]}
    assert selected_tss_ids == {str(tss_id) for tss_id in theme_a}
...
def test_strata_scope_must_include_conflict_still_selected_and_flagged(
    ...
    assert str(theme_b[0]) in selected_tss_ids
...
def test_exclude_ids_removes_docs_from_selectable_pool(conn: Connection) -> None:
    ...
    selected_tss_ids = {record["tss_id"] for record in row._mapping["selected"]}
    assert str(theme_a[0]) not in selected_tss_ids
    assert selected_tss_ids == {str(tss_id) for tss_id in theme_a[1:]}
    assert summary["base"]["excluded_by_directive"] == 1
    assert summary["base"]["eligible"] == 2
    assert {"tss_id": str(theme_a[0]), "flag": "excluded_by_directive"} in (
...
def test_trigger_flag_thin_base(conn: Connection) -> None:
    ...
    conn.execute(
        ...
        .where(source_screening_result.c.task_source_snapshot_id.in_(docs))
...
def test_rationale_bidirectional_with_hand_computed_full_text_shares(conn: Connection) -> None:
    ...
    assert set(strata_by_name["A"]["selected_ids"]) == {str(tss_id) for tss_id in theme_a_full}
...
def test_harness_select_component_missing_characterisation_fails(conn: Connection) -> None:
    pid, other_run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
...
def test_delete_task_data_removes_selection_result(conn: Connection) -> None:
    pid, characterise_run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    docs = _docs(conn, pid, characterise_run_id, scope_id, "A", 2)
    seed_characterisation(conn, pid, scope_id, characterise_run_id, themes={"A": docs})
    ...
    count = conn.execute(
        select(sa.func.count())
        .select_from(selection_result)
        .where(selection_result.c.task_id == pid)
    ).scalar_one()
    assert count == 0
...
def test_select_writes_no_artefact_or_block_and_leaves_screening_untouched(
    ...
    screening_before = conn.execute(
        ...
        .order_by(source_screening_result.c.task_source_snapshot_id)
    ).fetchall()

    run_select(conn, pid, scope_id, characterise_run_id, context={"selection": {"budget": 2}})

    artefact_count = conn.execute(
        select(sa.func.count()).select_from(artefact).where(artefact.c.task_id == pid)
    ).scalar_one()
    block_count = conn.execute(
        ...
        .where(
            block.c.artefact_id.in_(
                select(artefact.c.artefact_id).where(artefact.c.task_id == pid)
            )
        )
    ).scalar_one()
