...
def _seed_doc(
    ...
    *,
    ...
    _, tss_id = seed_source(conn, task_id, meta={"title": title, "abstract": abstract})
...
def test_coverage_distributions_match_hand_computed(conn: Connection) -> None:
    ...
    _, tss_not_relevant = seed_source(conn, pid, meta={"abstract": "Off-topic."})
    _, tss_failed = seed_source(conn, pid)

    for tss in (pss1, pss2, pss3):
        seed_screening_result(conn, pid, rid, scope_id, tss, status="relevant")
    seed_screening_result(conn, pid, rid, scope_id, tss_not_relevant, status="not_relevant")
    conn.execute(source_screening_result.insert().values(
        source_screening_result_id=uuid.uuid4(),
        evidence_scope_id=scope_id,
        task_source_snapshot_id=tss_failed,
        task_id=pid,
...
def test_base_counts_effective_grain_four_shapes(conn: Connection) -> None:
    ...
    Raw row counts would double-count a confirmed doc against task_sources
    and go negative on unscreened for a failed-then-retried doc; both must be
    counted exactly once, at their effective status.
    """
    from policy_atlas.evidence_search.corpus.characterise import _base_counts
    ...
    # once, never double-counted against task_sources, unscreened never negative.
...
def test_coverage_tag_distribution_keeps_asserters_separate(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    tss_id = _seed_doc(conn, pid, rid, scope_id, title="Housing report",
...
def test_overton_mixed_topic_shapes_both_materialise(conn: Connection) -> None:
    ...
    tags = {
        ...
        for row in conn.execute(
            ...
            .where(source_tag.c.task_id == pid)
...
def test_ingest_upload_materialises_no_provider_tags(conn: Connection) -> None:
    ...
    ingest_upload(
        conn, task_id=pid, chunks=["Some uploaded text."],
        source_locator="doc.pdf", metadata={}, text_basis="full_text",
    )
    count = conn.execute(
        select(sa.func.count()).select_from(source_tag).where(source_tag.c.task_id == pid)
    ).scalar_one()
    assert count == 0
...
def test_provider_fields_retained_unchanged_on_snapshot(conn: Connection) -> None:
    ...
    row = conn.execute(
        ...
        .where(task_source_snapshot.c.task_id == pid)
        .limit(1)
    ).scalar_one()
...
def test_grouping_happy_path_themes_and_unclustered(conn: Connection) -> None:
    ...
    for theme in theme_docs:
        for i in range(2):
            ...
            theme_docs[theme].append(tss_id)
    unclustered_tss = _seed_doc(conn, pid, rid, scope_id, title="Zulu Report")
    ...
    row = conn.execute(
        select(characterisation_result).where(characterisation_result.c.task_id == pid)
    ).one()
    member_ids_by_theme = {t["name"]: set(t["member_ids"]) for t in row.themes["themes"]}
    for theme, tss_ids in theme_docs.items():
        assert member_ids_by_theme[theme] == {str(p) for p in tss_ids}
    assert set(row.themes["unclustered_ids"]) == {str(unclustered_tss)}
...
def test_theme_tags_persist_accrete_and_scope_run_is_unique(conn: Connection) -> None:
    pid, rid1 = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)

    grouped_tss = _seed_doc(
        conn, pid, rid1, scope_id, title="Health report", abstract="Body. [stub-theme: Health]"
    )
    unclustered_tss = _seed_doc(conn, pid, rid1, scope_id, title="Zulu report")
    ...
    assert tag_rows[0].task_source_snapshot_id == grouped_tss
    assert tag_rows[0].tag == "Health"

    unclustered_tags = conn.execute(
        select(sa.func.count())
        .select_from(source_tag)
        .where(source_tag.c.task_source_snapshot_id == unclustered_tss)
        .where(source_tag.c.asserted_by == "characterise")
    ).scalar_one()
    ...
    char_rows = conn.execute(
        ...
        .where(characterisation_result.c.task_id == pid)
    ).scalar_one()
    assert char_rows == 2
...
def test_characterise_guidance_flows_to_provenance_and_discovery_only(
    ...
    summary = characterise_scope(
        conn, task_id=pid, run_id=rid, context=ctx, theme_grouping_backend=backend,
    )
...
def test_coverage_two_scopes_one_task_isolated(conn: Connection) -> None:
    """Two evidence scopes in ONE task: screened counts, distributions and
    themes are scope-local (no cross-scope bleed), while the coverage BASE is
    the whole task pool by design — a doc screened only in the other scope
    ...
    _, tss_a1 = seed_source(conn, pid, meta={
        "language": "en", "abstract": "On housing. [stub-theme: Housing]",
    })
    _, tss_a2 = seed_source(conn, pid, meta={
        "language": "en", "abstract": "More housing. [stub-theme: Housing]",
    })
    _, tss_a_not_relevant = seed_source(conn, pid, meta={"abstract": "Off-topic for A."})

    _, tss_b1 = seed_source(conn, pid, meta={
        "language": "fr", "abstract": "Sur la sante. [stub-theme: Health]",
    })

    for tss in (tss_a1, tss_a2):
        seed_screening_result(conn, pid, rid, scope_a, tss, status="relevant")
    seed_screening_result(conn, pid, rid, scope_a, tss_a_not_relevant, status="not_relevant")
    seed_screening_result(conn, pid, rid, scope_b, tss_b1, status="relevant")
    ...
    # FOR SCOPE A (task-pool base), never counted as screened.
    ...
    assert member_ids_a == {str(tss_a1), str(tss_a2)}
    ...
    assert member_ids_b == {str(tss_b1)}
...
def test_discovery_failure_raises_with_coverage_and_persists_nothing(conn: Connection) -> None:
    ...
    with capture_logs() as logs, pytest.raises(CharacteriseFailure) as exc_info:
        characterise_scope(
            conn, task_id=pid, run_id=rid, context=ctx,
...
def test_delete_task_data_removes_characterisation_and_tags(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _seed_doc(conn, pid, rid, scope_id, title="Report", abstract="Body. [stub-theme: X]")
    ...
    for table in (characterisation_result, source_tag):
        count = conn.execute(
            select(sa.func.count()).select_from(table).where(table.c.task_id == pid)
        ).scalar_one()
        assert count == 0
...
def test_downstream_outputs_unchanged_by_characterise(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    _, tss_id = seed_source(conn, pid, meta={
...
def test_judgment_invented_assignment_ids_are_dropped_without_repair(
    ...
    tag_tss_ids = {
        str(row.task_source_snapshot_id)
        for row in conn.execute(
            select(source_tag.c.task_source_snapshot_id)
            .where(source_tag.c.task_id == pid)
            .where(source_tag.c.asserted_by == "characterise")
        )
    }
    assert backend.invented_id not in tag_tss_ids
...
def test_characterise_persists_matching_theme_identity_on_result_and_tags(
    ...
    themes = conn.execute(
        select(characterisation_result.c.themes).where(characterisation_result.c.task_id == pid)
    ).scalar_one()
...
def test_judgment_openai_key_hygiene(
    ...
    payloads = conn.execute(
        select(event_log.c.payload).where(event_log.c.task_id == pid)
    ).fetchall()
...
def test_stub_runs_byte_identical(conn: Connection) -> None:
    pid, rid1 = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    tss_ids = _seed_three_docs(conn, pid, rid1, scope_id)
    for tss_id, theme in zip(tss_ids, ("Housing", "Health", "Education"), strict=True):
        conn.execute(source_tag.insert().values(
            source_tag_id=uuid.uuid4(),
            task_id=pid,
            task_source_snapshot_id=tss_id,
