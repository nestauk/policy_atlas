...
The section-drafting prompt bumps to ``synthesise_section_v10`` (always), with
...
def test_section_prompt_version_is_v10() -> None:
...
def _seed_finding(conn: Connection) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID, uuid.UUID]:
    ...
    snap_id, tss_id = seed_source(conn, task_id, meta={"title": "Doc"})
    ...
    _record_id, finding_id = _seed_reader_finding(
        ...
        tss_id=tss_id, snap_id=snap_id,
    )
    return task_id, run_id, scope_id, finding_id


def test_query_findings_records_carry_mark_when_annotated(conn: Connection) -> None:
    task_id, run_id, scope_id, finding_id = _seed_finding(conn)
    reader = make_findings_reader(
        conn, task_id=task_id, extraction_run_id=run_id,
...
def _extraction_row(tss_id: str, record_id: str, annotations: dict[str, str]) -> dict[str, Any]:
...
def test_load_findings_member_record_carries_mark(conn: Connection) -> None:
    ...
    row = _extraction_row(str(tss_id), str(record_id), annotations)
    assert _relevance_annotations(row) == annotations

    finding_by_id, _icf, _avail, _bases = _load_findings(
        conn, task_id=task_id, extraction_row=row, relevance_annotations=annotations
    )
    ...
    bare_row = _extraction_row(str(tss_id), str(record_id), {})
    finding_by_id2, _i2, _a2, _b2 = _load_findings(
        conn, task_id=task_id, extraction_row=bare_row, relevance_annotations={}
    )
