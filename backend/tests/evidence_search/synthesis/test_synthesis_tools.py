...
from policy_atlas.evidence_search.synthesis import synthesis_tools
...
def test_lookup_excludes_screened_out_doc_from_tag_reads(conn: Connection) -> None:
    ...
    from policy_atlas.core.schema import TOPIC_THEME, source_screening_result, source_tag
...
def test_lookup_screening_by_doc_reaches_screening_rows(conn: Connection) -> None:
    ...
    result = reader({"kind": "screening_by_doc", "doc_id": str(tss_id)})
...
def test_search_chunks_result_carries_default_metadata_set_present_and_absent(
    ...
    from policy_atlas.core.schema import source_snapshot

    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)

    # Rich doc: classification + appraisal + year + venue + cited_by.
    rich_tss = seed_select_doc(
        conn, task_id, run_id, scope_id, title="Rich metadata doc", year=2024
    )
    rich_snap = conn.execute(
        select(task_source_snapshot.c.source_snapshot_id).where(
            task_source_snapshot.c.task_source_snapshot_id == rich_tss
        )
    ).scalar_one()
    conn.execute(
    ...
    bare_snap, bare_tss = seed_source(
        conn, task_id, meta={"title": "Bare doc"}
    )
    seed_screening_result(conn, task_id, run_id, scope_id, bare_tss, status="relevant")
...
def test_make_findings_reader_record_carries_default_metadata_set(
    ...
    (scoped to task + evidence scope, outerjoin) and attaches the default
...
def test_make_findings_reader_evidence_type_reads_live_classification_not_provenance(
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    snap_id, tss_id = seed_source(conn, task_id, meta={"title": "Divergent doc"})
    seed_screening_result(conn, task_id, run_id, scope_id, tss_id, status="relevant")
...
def test_make_findings_reader_returns_kind_segregated_sections(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    snap_id, tss_id = seed_source(conn, task_id, meta={"title": "Both kinds"})
