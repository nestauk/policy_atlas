...
read-only tool set, per-section citation isolation, foreign-task scope guards,
...
def test_sibling_repair_guard(conn: Connection) -> None:
    ...
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="rough sleeping evidence")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
...
def test_injection_shaped_chunk_and_tag_land_inert(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="injection fixture")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
        chunks=[
...
# --- Test 5: foreign-task scope guard ---


def test_foreign_task_scope_guard(conn: Connection) -> None:
    task_a, run_a = seed_task_and_run(conn)
    scope_a = seed_scope(conn, task_a)
    tss_a = seed_select_doc(conn, task_a, run_a, scope_a, title="task A doc")
    seed_ingested_full_text(conn, tss_id=tss_a, chunks=["Alpha corpus evidence chunk."])

    task_b, run_b = seed_task_and_run(conn)
    scope_b = seed_scope(conn, task_b)
    tss_b = seed_select_doc(conn, task_b, run_b, scope_b, title="task B doc")
    seed_ingested_full_text(conn, tss_id=tss_b, chunks=["Beta corpus evidence chunk."])

    scope = build_retrieval_scope(
        conn, task_id=task_a, scope_id=scope_a, selected_tss_ids=set()
    )
    # No unit or chunk of B is reachable from A's retrieval scope.
    assert str(tss_b) not in scope.docs
    assert all(unit["tss_id"] != str(tss_b) for unit in scope.units)
    assert all(chunk["tss_id"] != str(tss_b) for chunk in scope.chunks.values())
    ...
    with pytest.raises(ToolValidationError):
        reader({"kind": "tags_by_doc", "doc_id": str(tss_b)})
...
def test_screened_out_doc_unreachable(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="screened out doc")
    ...
    seed_screening_result(conn, task_id, run_id, scope_id, tss_id, status="not_relevant")
    seed_ingested_full_text(conn, tss_id=tss_id, chunks=["Unreachable screened-out text."])
...
def test_demoted_doc_unreachable_in_retrieval_scope(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="demoted doc")
    # seed_select_doc already seeded the stage-1 relevant row; add the stage-2
    # demotion on top of it.
    seed_screening_result(
        conn, task_id, run_id, scope_id, tss_id, status="not_relevant", screen_stage=2
    )
    seed_ingested_full_text(conn, tss_id=tss_id, chunks=["Demoted at stage 2."])
    ...
    with pytest.raises(ToolValidationError):
        reader({"kind": "tags_by_doc", "doc_id": str(tss_id)})
...
def test_ledger_cross_section_citation_rejected(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="cross section corpus")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
    ...
    assert _count(conn, citation, task_id) == 1
...
def test_socket_deny_synthesise_harness_round_trip(
    ...
    task_id, char_run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    seed_characterisation(
        conn, task_id, scope_id, char_run_id, themes={"theme-a": []}
    )
...
def _empty_substrate() -> Any:
    from policy_atlas.evidence_search.synthesis.synthesise import CorpusProfile, SubstrateView
...
def test_reasoning_over_cap() -> None:
    from policy_atlas.evidence_search.synthesis.synthesise import validate_claims
...
def test_judge_coverage_violation_fails_honestly(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="judge coverage corpus")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
