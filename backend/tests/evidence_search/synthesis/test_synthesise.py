...
from policy_atlas.evidence_search.extract.quote_verify import build_basis
...
from policy_atlas.evidence_search.synthesis.synthesis_tools import SECTION_CAP, ToolExchange
...
from policy_atlas.runtime.agent import persist_approved_plan
from policy_atlas.runtime.task_plan import TaskPlan
from tests.evidence_search.group.test_group import (
    _group_row,
    seed_extraction,
)
from tests.evidence_search.group.test_group import (
...
def test_artefact_records_walkable_conversation_plan_run_lineage(conn: Connection) -> None:
    ...
    seed_select_doc(conn, task_id, run_id, scope_id, title="Lineage source")
    ...
    lineage = conn.execute(
        ...
        .join(task_plan, task_plan.c.conversation_id == conversation.c.id)
        .join(
            capability_run,
            (capability_run.c.plan_id == task_plan.c.plan_id)
            & (capability_run.c.plan_version == task_plan.c.version),
        )
    ...
    assert conn.execute(
        select(task_plan.c.conversation_id).where(
            task_plan.c.plan_id == legacy_plan_id
        )
    ).scalar_one() is None
    assert conn.execute(
...
def test_unmatched_retrieval_boost_recorded_never_fatal(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Boost doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
...
def test_determinism_two_runs_identical_content(conn: Connection) -> None:
    task_id, run_id_1 = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    characterisation_run_id = seed_run(conn, task_id)
    seed_characterisation(
        conn,
...
def test_boundary_spanning_quote_writes_row_per_chunk(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Boundary doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
    ...
    assert _count(conn, citation, task_id) == 2
...
def test_judge_persistence_keys_on_cited_claims_only(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Judge doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
        chunks=[
...
def test_delete_task_data_after_synthesise(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Delete-safe doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
        chunks=[
    ...
    _run_synthesise(conn, task_id=task_id, run_id=run_id, scope_id=scope_id)
    assert _count(conn, synthesis_result, task_id) == 1
    assert _count(conn, artefact, task_id) == 1
...
def test_gap_corpus_caveat_and_degradation(conn: Connection) -> None:
    ...
    acquired_run_id_1 = seed_run(conn, task_id)
    conn.execute(
    ...
    acquired_run_id_2 = seed_run(conn, task_id)
    conn.execute(
        search_coverage_record.insert().values(
    ...
    degraded_payloads = [
        p
        for p in _task_annotations(conn, task_id)
...
def test_corpus_profile_excludes_demoted_doc(conn: Connection) -> None:
    ...
    from policy_atlas.evidence_search.synthesis.synthesise import _load_corpus_profile

    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)

    seed_select_doc(conn, task_id, run_id, scope_id, title="confirmed")

    _, demoted_tss = seed_source(conn, task_id, meta={"title": "demoted"})
    seed_screening_result(
    ...
    profile = _load_corpus_profile(conn, task_id=task_id, scope_id=scope_id)
...
def test_screened_chunks_excludes_demoted_doc(conn: Connection) -> None:
    ...
    confirmed_tss = seed_select_doc(conn, task_id, run_id, scope_id, title="confirmed")
    seed_ingested_full_text(conn, tss_id=confirmed_tss, chunks=["Confirmed doc body."])
    ...
    seed_ingested_full_text(conn, tss_id=demoted_tss, chunks=["Demoted doc body."])

    _chunk_by_id, chunks_by_tss, _basis_by_tss = _load_screened_chunks(
        conn,
        task_id=task_id,
        scope_id=scope_id,
        selected_tss_ids=set(),
    ...
    assert str(confirmed_tss) in chunks_by_tss
    assert str(demoted_tss) not in chunks_by_tss
...
def test_fresh_single_facet_group_shape_is_consumed_by_synthesise(
    ...
    task_id, synthesis_run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    ...
    characterisation_run_id = seed_run(conn, task_id)
    seed_characterisation(
        conn, task_id, scope_id, characterisation_run_id, themes={"theme-a": []}
    )
    conn.execute(
    ...
    grouping_row = _group_row(conn, task_id, grouping_run_id)
...
def test_conclusions_and_key_findings_composition(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Composition doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
        chunks=[
...
def test_key_findings_absence_path_mints_no_block(conn: Connection) -> None:
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Absence doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
...
def test_key_findings_claim_citing_uncited_id_is_rejected_and_counted(
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Union doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
...
def test_load_findings_carries_effect_basis_and_study_geography(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    snap_id, tss_id = seed_source(conn, task_id, meta={"title": "Geo doc"})
...
def test_load_findings_tolerates_v1_null_rows(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    snap_id, tss_id = seed_source(conn, task_id, meta={"title": "V1 doc"})
    record_id = uuid.uuid4()
    finding_id = uuid.uuid4()
    _seed_iof_row(
        conn,
...
def test_load_findings_batches_chunk_basis_query(conn: Connection) -> None:
    ...
    for index, chunks in enumerate([
        ...
        snap_id, tss_id = seed_source(conn, task_id, meta={"title": f"Doc {index}"})
        record_id = uuid.uuid4()
        finding_id = uuid.uuid4()
    ...
    snap_id, tss_id = seed_source(
        conn, task_id, meta={"title": "Doc 2", "abstract": "The chunkless abstract."}
    )
    record_id = uuid.uuid4()
    finding_id = uuid.uuid4()
    record_ids.append(record_id)
    ...
    try:
        findings, _icf_findings, _icf_available, basis_by_snapshot = _load_findings(
            conn, task_id=task_id, extraction_row=extraction_row
        )
    finally:
...
def _seed_group_with_findings(
    ...
    Returns:
        ``(task_id, run_id, scope_id, extraction_run_id, grouping_run_id,
        finding_ids)``; ``finding_ids[i]`` corresponds to ``findings[i]``.
    """
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Finding doc")
    characterisation_run_id = seed_run(conn, task_id)
    seed_characterisation(
        conn, task_id, scope_id, characterisation_run_id, themes={"theme-a": [tss_id]}
    )
    _, _, selection_run_id = run_select(conn, task_id, scope_id, characterisation_run_id)
    ...
    return task_id, run_id, scope_id, extraction_run_id, grouping_run_id, finding_ids
...
def _seed_group_with_iof_and_icf(
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="ICF doc")
...
def test_key_findings_seed_carries_only_cited_only_chunk_content(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="Rider-16 doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
        chunks=[
...
def _runs_count(conn: Connection, task_id: uuid.UUID) -> int:
    return int(
        conn.execute(
            select(func.count()).select_from(runs).where(runs.c.task_id == task_id)
        ).scalar_one()
    )
...
def test_propose_synthesis_plan_is_side_effect_free(conn: Connection) -> None:
    ...
    task_id, _run_id, scope_id, extraction_run_id, grouping_run_id, _finding_ids = (
        _seed_group_with_findings(conn, [{"intervention": "Alpha service"}])
    )

    before = {
        "artefact": _count(conn, artefact, task_id),
        "synthesis_result": _count(conn, synthesis_result, task_id),
        "block": _count(conn, block, task_id),
        "annotation": _count(conn, annotation, task_id),
        "addressable_unit": _count(conn, addressable_unit, task_id),
        "citation": _count(conn, citation, task_id),
        "runs": _runs_count(conn, task_id),
    }
...
def test_propose_synthesis_plan_output_schema(conn: Connection) -> None:
    ...
    task_id, _run_id, scope_id, extraction_run_id, grouping_run_id, finding_ids = (
        _seed_group_with_findings(conn, [{"intervention": "Alpha service"}])
    )
...
def test_propose_then_compile_directive_round_trips_into_a_run(conn: Connection) -> None:
    ...
    task_id, run_id, scope_id, extraction_run_id, grouping_run_id, _finding_ids = (
        _seed_group_with_findings(conn, [{"intervention": "Alpha service"}])
    )
...
def test_case_studies_present_composition(conn: Connection) -> None:
    ...
    characterisation_run_id = seed_run(conn, task_id)
    seed_characterisation(
        conn, task_id, scope_id, characterisation_run_id, themes={"nutrition": []}
    )
    conn.execute(
        update(selection_result)
        .where(selection_result.c.run_id == seeded.selection_run_id)
    ...
    # made every card task onto the same citations in the read model.
...
def test_case_studies_absent_composition(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="No-CS doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
...
def test_mrs_notes_present_in_rollup(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = seed_select_doc(conn, task_id, run_id, scope_id, title="MRS doc")
    seed_ingested_full_text(
        conn,
        tss_id=tss_id,
        chunks=[
    ...
    # or findings). With chunk claims, the substrate maps chunk→tss, so notes
