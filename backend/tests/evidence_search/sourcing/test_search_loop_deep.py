...
def _search_payloads(conn: Connection, task_id: uuid.UUID) -> list[dict[str, Any]]:
    return [
        event["payload"]
...
def _coverage_rows(conn: Connection, task_id: uuid.UUID) -> list[Any]:
    return list(
        conn.execute(
...
def _screening_count(conn: Connection, task_id: uuid.UUID) -> int:
    return int(
        conn.execute(
...
def _seed_screened_source(
    ...
    *,
    ...
    _, tss_id = seed_source(conn, task_id, meta=metadata)
    _seed_screening(
        conn,
        task_id=task_id,
        run_id=run_id,
...
def test_rapid_fanout_events_failed_variant_isolated(conn: Connection) -> None:
    ...
    payloads = _search_payloads(conn, task_id)
...
def test_generation_failure_raises_and_harness_records_component_failed(
    ...
    harness_run_id = seed_run(conn, task_id)
    run_harness(
        conn,
...
def test_zero_result_generated_queries_count_and_openalex_fallback(
    ...
    openalex_search_events = [
        payload for payload in _search_payloads(conn, task_id)
...
def test_fanout_runs_every_planned_call_with_no_time_budget(
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id, context={"search": {"depth": "standard"}})
    generation = ScriptedGenerationBackend(
...
def test_deep_exemplar_payload_re_reads_effective_state_between_rounds(
    ...
    task_id, first_run = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id, context={"search": {"depth": "deep"}})
    _seed_coverage_row(
        conn,
        task_id=task_id,
        scope_id=scope_id,
...
def test_standard_round_two_trims_snowball_and_suggest_arms(
    ...
    origins = {payload["query_origin"] for payload in _search_payloads(conn, task_id)}
    assert not origins & {
        "snowball_forward",
        "snowball_backward",
        "suggestion_doi",
...
def test_suggestion_grounding_matrix_and_screened_out_counter(
    ...
    assert counts["acquired_tss_by_verb"]["lookup_dois"]
    assert counts["acquired_tss_by_verb"]["lookup_title"]
    assert len(counts["acquired_tss_by_verb"]["lookup_dois"]) == 2
    assert len(counts["acquired_tss_by_verb"]["lookup_title"]) == 1
...
def test_search_guidance_leaves_evidence_scope_row_unchanged(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, task_id, context=directive)

    config = compile(Plan(component="acquire", evidence_scope_id=scope_id))
    events.append(conn, task_id=task_id, run_id=run_id, event_type="run.started", payload={})
    events.append(
        conn, task_id=task_id, run_id=run_id, event_type="plan.compiled", payload={}
    )
    run_harness(
        conn, config=config, task_id=task_id, run_id=run_id, provider=StubEchoProvider()
    )
...
def test_finalise_deep_stop_writes_raw_condition_when_not_thin(
    ...
    task_id, run_id = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    _seed_coverage_row(conn, task_id=task_id, scope_id=scope_id, run_id=run_id)
    ...
    assert _coverage_rows(conn, task_id)[-1].stop_condition == "budget_exhausted"
...
def test_finalise_deep_stop_thin_overlay_wins_over_raw_condition(
    ...
    assert _coverage_rows(conn, task_id)[-1].stop_condition == "re_searched_still_thin"
...
def test_finalise_deep_stop_targets_only_the_latest_coverage_row(
    ...
    rows = _coverage_rows(conn, task_id)
...
def test_new_confident_relevant_for_run_counts_only_that_runs_confident_rows(
    ...
    task_id, run_a = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    run_b = seed_run(conn, task_id)
    _seed_screened_source(
        conn, task_id=task_id, run_id=run_a, scope_id=scope_id,
    ...
    _seed_screened_source(
        conn, task_id=task_id, run_id=run_b, scope_id=scope_id,
        title="B confident", confidence=0.9,
    )

    assert new_confident_relevant_for_run(
        conn, task_id=task_id, scope_id=scope_id, run_id=run_a
    ) == 2
    assert new_confident_relevant_for_run(
        conn, task_id=task_id, scope_id=scope_id, run_id=run_b
    ) == 1
...
def test_acquire_search_does_not_write_screening_rows(
    ...
    before = _screening_count(conn, task_id)
    generation = ScriptedGenerationBackend(
    ...
    assert _screening_count(conn, task_id) == before
    assert (
        conn.execute(
            select(sa.func.count())
