...
def test_scov_cross_task_fk_rejected(conn: Connection) -> None:
    pid_a, rid_a = seed_task_and_run(conn)
    pid_b, _ = seed_task_and_run(conn)
    scope_b = seed_scope(conn, pid_b)
    # scope from task B with task A's id/run: composite FK must reject
    with pytest.raises(IntegrityError, match="fk_scov_scope_task"):
...
def test_acquire_round_trip_both_backends(conn: Connection) -> None:
    ...
    rows = conn.execute(
        select(task_source_snapshot, source_snapshot)
        .join(
            source_snapshot,
...
def test_acquired_metadata_provider_fields_url_oa_block(conn: Connection) -> None:
    ...
    rows = conn.execute(
        ...
        .select_from(task_source_snapshot.join(
            source_snapshot,
            task_source_snapshot.c.source_snapshot_id
            == source_snapshot.c.source_snapshot_id,
        ))
...
def test_acquire_idempotent_rerun(conn: Connection) -> None:
    ...
    n_snapshots = conn.execute(
        select(sa.func.count()).select_from(task_source_snapshot)
        .where(task_source_snapshot.c.task_id == pid)
    ).scalar_one()
    ...
    n_cov = conn.execute(
        ...
        .where(search_coverage_record.c.task_id == pid)
    ).scalar_one()
    assert n_cov == 2
...
def test_cross_task_dedup_isolation(conn: Connection) -> None:
    """Task B acquiring the same fixtures still gets its own links; A untouched."""
    pid_a, rid_a = seed_task_and_run(conn)
    scope_a = seed_scope(conn, pid_a)
    counts_a = acquire(conn, pid_a, rid_a, scope_a)

    pid_b, rid_b = seed_task_and_run(conn)
    scope_b = seed_scope(conn, pid_b)
    counts_b = acquire(conn, pid_b, rid_b, scope_b)

    assert counts_b["acquired"] == counts_a["acquired"] == 24
    assert counts_b["already_acquired"] == 0  # dedup never consults task A's rows
...
def test_source_acquired_events(conn: Connection) -> None:
    ...
    for e in acquired_events:
        payload = e["payload"]
        assert set(payload) == {
            "source_snapshot_id", "task_source_snapshot_id",
...
def test_delete_task_data_removes_acquired_rows(conn: Connection) -> None:
    pid, rid = seed_task_and_run(conn)
    scope_id = seed_scope(conn, pid)
    acquire(conn, pid, rid, scope_id)
    ...
    for table in (task_source_snapshot, search_coverage_record):
        n = conn.execute(
            select(sa.func.count()).select_from(table)
            .where(table.c.task_id == pid)
        ).scalar_one()
        assert n == 0
    n_tasks = conn.execute(
        select(sa.func.count()).select_from(task).where(task.c.task_id == pid)
    ).scalar_one()
    assert n_tasks == 0
...
def test_acquire_module_has_no_http_client() -> None:
    ...
    live_import = re.compile(
        r"^\s*(import|from)\s+policy_atlas\.evidence_search\.sourcing\.search_live\b|"
        r"^\s*from\s+policy_atlas\.evidence_search\.sourcing\s+import\s+.*\bsearch_live\b",
        re.MULTILINE,
    )
    acquire_text = (src_dir / "evidence_search" / "sourcing" / "acquire.py").read_text()
    assert not live_import.search(acquire_text), "acquire.py imports the live module"

    fetch_live_import = re.compile(
        r"^\s*(import|from)\s+policy_atlas\.evidence_search\.sourcing\.fetch_live\b|"
        r"^\s*from\s+policy_atlas\.evidence_search\.sourcing\s+import\s+.*\bfetch_live\b",
        re.MULTILINE,
    )
    ingest_text = (src_dir / "evidence_search" / "sourcing" / "ingest_full_text.py").read_text()
...
def test_provider_tag_bounds() -> None:
    from policy_atlas.evidence_search.sourcing.acquire import TAG_MAX_LENGTH, _normalize_tag
