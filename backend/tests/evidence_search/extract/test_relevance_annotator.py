...
def _seed_two_finding_doc(
    ...
    *,
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Doc",
...
def test_extraction_and_vetter_payloads_byte_identical_with_or_without_emphasis(
    ...
    Two independent tasks (so both fresh-extract, no memo interference); the
    ...
    # Task A — no emphasis.
    pa, sel_a = seed_task_and_run(conn)
    scope_a = seed_scope(conn, pa)
    tss_a, cid_a, findings_a = _seed_two_finding_doc(conn, pa, sel_a, scope_a)
    ...
    # Task B — emphasis + annotator.
    pb, sel_b = seed_task_and_run(conn)
    scope_b = seed_scope(conn, pb)
    tss_b, cid_b, findings_b = _seed_two_finding_doc(conn, pb, sel_b, scope_b)
    ...
    # Fencing property #2 — byte-identity: normalise the per-doc ids (tss +
    ...
    def _norm(payloads: list[str], tss: str, cid: str) -> list[str]:
        return [p.replace(tss, "<TSS>").replace(cid, "<CHUNK>") for p in payloads]

    assert _norm(ext_a.payloads, str(tss_a), str(cid_a)) == _norm(
        ext_b.payloads, str(tss_b), str(cid_b)
    )
    assert _norm(vet_a.payloads, str(tss_a), str(cid_a)) == _norm(
        vet_b.payloads, str(tss_b), str(cid_b)
    )
...
def test_fingerprint_excludes_emphasis_via_memo_hit(conn: Connection) -> None:
    ...
    _, _cid, findings = _seed_two_finding_doc(conn, task_id, sel_run, scope_id)
    ...
    # Run B — WITH emphasis — same task/doc → memo HIT (fresh backend that
