...
import policy_atlas.evidence_search.extract.iof_prompt as iof_prompt
...
def test_injection_component_level_is_inert_data(conn: Connection) -> None:
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Injected doc", chunk_content=INJECTION
    )
    _seed_selection(
...
def test_injection_hijacked_model_stored_as_data(conn: Connection) -> None:
    ...
    tss_id, chunk_id = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Injected doc",
    ...
    summary, _ = _run_backend(
        conn, task_id, scope_id, sel_run, _HijackedBackend(str(cid))
    )
...
def test_prompt_structure_carries_envelope_and_no_intent(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, chunk_id = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Cohort study of X",
...
def test_counting_only_fresh_docs_sent_and_calls_match_budget(conn: Connection) -> None:
    ...
    task_id, sel_run1 = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    a_cid = uuid.uuid4()
    a_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run1, scope_id, title="Doc A",
    ...
    b_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run1, scope_id, title="Doc B", chunk_content="Body B.",
    )
    c_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run1, scope_id, title="Doc C", chunk_content="Body C.",
    )

    # First run extracts only A, so its record memoises.
    _seed_selection(
        conn, task_id, sel_run1, scope_id, [{"tss_id": str(a_tss), "text_basis": "full_text"}]
    )
    _run(conn, task_id, scope_id, sel_run1)

    # Second run selects A, B, C — A is a memo hit, B and C are fresh.
    sel_run2 = seed_run(conn, task_id)
    _seed_selection(conn, task_id, sel_run2, scope_id, [
        {"tss_id": str(a_tss), "text_basis": "full_text"},
        {"tss_id": str(b_tss), "text_basis": "full_text"},
        {"tss_id": str(c_tss), "text_basis": "full_text"},
    ])
    backend = _RecordingBackend(StubExtractionBackend())
    summary, _ = _run_backend(conn, task_id, scope_id, sel_run2, backend)

    assert profile_counts(summary)["reused"] == 1
    assert profile_counts(summary)["fresh"] == 2
    sent = {payload.tss_id for payload in backend.payloads}
    assert sent == {str(b_tss), str(c_tss)}  # the reused doc's tss never appears
...
def test_misbehaving_duplicate_findings_dedup(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Dup doc",
    ...
    summary, _ = _run_backend(conn, task_id, scope_id, sel_run, backend)

    assert profile_doc(summary)["finding_count"] == 1
    assert profile_findings(summary)["dedup_collapsed"] == 1
    assert len(_findings(conn, task_id)) == 1
...
def test_misbehaving_null_string_numerics_coerced(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Numeric doc",
    ...
    _run_backend(conn, task_id, scope_id, sel_run, backend)
...
def test_misbehaving_oversized_output_all_stored(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Big doc",
        chunk_content="Body.", chunk_id=cid,
    )
    _seed_selection(
        conn, task_id, sel_run, scope_id, [{"tss_id": str(tss_id), "text_basis": "full_text"}]
    ...
    assert len(_findings(conn, task_id)) == 200
...
def test_repeated_quote_grounds_to_successive_occurrences(conn: Connection) -> None:
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Repeat doc",
...
def test_socket_deny_extract_round_trip(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, task_id)
    cid = uuid.uuid4()
    ft_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Full doc",
    ...
    ab_tss = _seed_abstract_doc(
        conn, task_id, title="Abstract doc",
    ...
    _seed_selection(conn, task_id, sel_run, scope_id, [
        {"tss_id": str(ft_tss), "text_basis": "full_text"},
        {"tss_id": str(ab_tss), "text_basis": "abstract_only"},
    ])
...
def test_key_hygiene_canary_absent_from_all_output(
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Key doc",
    ...
    findings = _findings(conn, task_id)
    row_dump = json.dumps(
