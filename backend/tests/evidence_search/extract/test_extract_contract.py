...
from policy_atlas.evidence_search.extract.extraction_backend import StubExtractionBackend
from policy_atlas.evidence_search.extract.iof_prompt import EXTRACT_SYSTEM_PROMPT
...
def _insert_extraction_record(
    ...
    **over: Any,
    ...
    values = _extraction_record_values(task_id, run_id, tss_id, snapshot_id, **over)
...
def _seed_multi_chunk_doc(
    ...
    *,
    ...
    segment_id. Returns ``(tss_id, chunk_ids)`` in sequence order.
    """
    ...
    conn.execute(task_source_snapshot.insert().values(
        task_source_snapshot_id=tss_id, task_id=task_id,
...
def _seed_determinism_fixture(
    ...
    pss1, _ = _seed_full_text_doc(
        conn, task_id, run_id, scope_id, title="Det doc one",
    ...
    pss2 = _seed_abstract_doc(
        conn, task_id, title="Det doc two",
    ...
    pss3, _ = _seed_full_text_doc(
        conn, task_id, run_id, scope_id, title="Det doc three",
        chunk_content="This document reports no intervention outcome findings.",
    )
    _seed_selection(conn, task_id, run_id, scope_id, [
        {"tss_id": str(pss1), "text_basis": "full_text"},
        {"tss_id": str(pss2), "text_basis": "abstract_only"},
        {"tss_id": str(pss3), "text_basis": "full_text"},
    ])
...
def test_ser_status_check_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_ser_status"), conn.begin_nested():
        conn.execute(source_extraction_record.insert().values(
            **_extraction_record_values(task_id, run_id, tss_id, snap_id, status="bogus")
        ))
...
def test_ser_basis_check_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_ser_basis"), conn.begin_nested():
        conn.execute(source_extraction_record.insert().values(
            **_extraction_record_values(task_id, run_id, tss_id, snap_id, basis="bogus")
        ))
...
def test_ser_error_presence_failed_without_error_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_ser_error_presence"), conn.begin_nested():
        conn.execute(source_extraction_record.insert().values(**_extraction_record_values(
            task_id, run_id, tss_id, snap_id, status="extraction_failed", error=None
        )))
...
def test_ser_error_presence_extracted_with_error_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_ser_error_presence"), conn.begin_nested():
        conn.execute(source_extraction_record.insert().values(**_extraction_record_values(
            task_id, run_id, tss_id, snap_id, status="extracted", error="should not be here"
        )))
...
def test_iof_effect_direction_check_rejected(conn: Connection) -> None:
    ...
    task_id, run_id = seed_task_and_run(conn)
    snap_id, tss_id = seed_source(conn, task_id)
    record_id = _insert_extraction_record(conn, task_id, run_id, tss_id, snap_id)
    with pytest.raises(IntegrityError, match="ck_iof_direction"), conn.begin_nested():
        conn.execute(intervention_outcome_finding.insert().values(
            **_finding_values(task_id, record_id, effect_direction="bogus")
        ))
...
def test_iof_estimate_level_check_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_iof_estimate_level"), conn.begin_nested():
        conn.execute(intervention_outcome_finding.insert().values(
            **_finding_values(task_id, record_id, estimate_level="bogus")
        ))
...
def test_iof_causality_check_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_iof_causality"), conn.begin_nested():
        conn.execute(intervention_outcome_finding.insert().values(
            **_finding_values(task_id, record_id, causality_by_design="bogus")
        ))
...
def test_iof_stratum_qualifiers_non_array_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_iof_strata_array"), conn.begin_nested():
        conn.execute(intervention_outcome_finding.insert().values(
            **_finding_values(
                task_id, record_id, stratum_qualifiers={"type": "timepoint", "value": "x"}
            )
        ))
...
def test_iof_grounding_non_array_rejected(conn: Connection) -> None:
    ...
    with pytest.raises(IntegrityError, match="ck_iof_grounding_array"), conn.begin_nested():
        conn.execute(intervention_outcome_finding.insert().values(
            **_finding_values(task_id, record_id, grounding={"quote": "x"})
        ))
...
def test_ser_memo_partial_unique_success_states_only(conn: Connection) -> None:
    ...
    _insert_extraction_record(
        conn, task_id, run_id, tss_id, snap_id, status="extracted", extraction_fingerprint=fp
    )
    with pytest.raises(IntegrityError, match="uq_ser_memo"), conn.begin_nested():
        conn.execute(source_extraction_record.insert().values(**_extraction_record_values(
            task_id, run_id, tss_id, snap_id, status="extracted", extraction_fingerprint=fp
        )))
    # Failures insert freely alongside a success row, and alongside each other.
    _insert_extraction_record(
        conn, task_id, run_id, tss_id, snap_id,
...
def test_iof_cross_task_composite_fk_rejected(conn: Connection) -> None:
    """A finding naming an extraction record from ANOTHER task is rejected."""
    task_a, run_a = seed_task_and_run(conn)
    task_b, _run_b = seed_task_and_run(conn)
    snap_id, tss_id = seed_source(conn, task_a)
    record_id = _insert_extraction_record(conn, task_a, run_a, tss_id, snap_id)
    with pytest.raises(IntegrityError, match="fk_iof_record_task"), conn.begin_nested():
        conn.execute(intervention_outcome_finding.insert().values(
            **_finding_values(task_b, record_id)
        ))
...
def test_extraction_result_unique_scope_run_rejected(conn: Connection) -> None:
    ...
    conn.execute(extraction_result.insert().values(
        extraction_result_id=uuid.uuid4(), task_id=task_id, evidence_scope_id=scope_id,
...
def test_failed_extraction_never_blocks_or_satisfies_memo(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, chunk_id = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Flaky doc", chunk_content=content,
    ...
    first, _ = _run(conn, task_id, scope_id, sel_run)
    ...
    second, _ = _run(conn, task_id, scope_id, sel_run)
...
def test_coverage_invariants_at_payload_boundary(conn: Connection) -> None:
    ...
    extracted_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Doc A",
    ...
    no_findings_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Doc B",
        chunk_content="This document reports nothing shaped like a finding.",
    )
    failed_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Doc C",
        chunk_content="Some content.", stub_failed=True,
    )
    selected_ids = {str(extracted_tss), str(no_findings_tss), str(failed_tss)}
    _seed_selection(conn, task_id, sel_run, scope_id, [
        {"tss_id": str(extracted_tss), "text_basis": "full_text"},
        {"tss_id": str(no_findings_tss), "text_basis": "full_text"},
        {"tss_id": str(failed_tss), "text_basis": "full_text"},
    ])

    summary, _ = _run(conn, task_id, scope_id, sel_run)

    assert {d["tss_id"] for d in summary["docs"]} == selected_ids
    ...
    for row in owning_tss:
        assert str(row.task_source_snapshot_id) in selected_ids
...
def test_fabricated_quote_kept_and_flagged(conn: Connection) -> None:
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Fab doc",
...
def test_chunk_id_is_verified_location_not_model_claim(conn: Connection) -> None:
    ...
    tss_id, chunk_ids = _seed_multi_chunk_doc(
        conn, task_id, sel_run, scope_id, title="Misclaimed doc",
    ...
    _run(conn, task_id, scope_id, sel_run)
...
def test_boundary_spanning_quote_verifies_across_chunks(conn: Connection) -> None:
    ...
    seeded_chunk_ids = [uuid.uuid4(), uuid.uuid4()]
    tss_id, chunk_ids = _seed_multi_chunk_doc(
        conn, task_id, sel_run, scope_id, title="Two-chunk doc", contents=[chunk1, chunk2],
        chunk_ids=seeded_chunk_ids,
        stub_iof=[_record(
...
def test_verified_anchor_match_location_fidelity(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Loc doc",
...
def test_field_rules_out_of_bounds_marks_unclear(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Bounds doc",
...
def test_null_like_string_coerced_to_null_and_marked(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Null-like doc",
...
def test_estimate_level_not_applicable_markers(conn: Connection) -> None:
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Estimate doc",
...
def test_claim_keyed_dedup_merges_anchors(conn: Connection) -> None:
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Dedup doc",
        chunk_content=content, chunk_id=cid,
        stub_iof=[
            _record(
...
def test_claim_dedup_distinct_effect_size_stays_separate(conn: Connection) -> None:
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="No dedup doc",
        chunk_content=content, chunk_id=cid,
        stub_iof=[
            _record(
                intervention="coaching", outcome="graduation rates",
...
def test_all_grain_invalid_doc_extraction_failed(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Invalid doc",
...
def test_windowing_multi_window_doc_extracts(
    ...
    tss_id, chunk_ids = _seed_multi_chunk_doc(
        conn, task_id, sel_run, scope_id, title="Windowed doc", contents=contents,
        chunk_ids=seeded_chunk_ids,
        stub_iof=[_record(
...
def test_oversize_chunk_subsegment_split(
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Oversize doc",
        chunk_content=content, chunk_id=cid,
        stub_iof=[_record(
...
def test_full_text_basis_mismatch_no_snapshot_no_chunks(conn: Connection) -> None:
    ...
    task_id, sel_run = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    _snap_id, tss_id = seed_source(
        conn, task_id, meta={"title": "No text doc", "abstract": "An abstract."}
    )
    _seed_selection(
        conn, task_id, sel_run, scope_id, [{"tss_id": str(tss_id), "text_basis": "full_text"}]
    )
...
def test_abstract_only_missing_abstract_is_empty_basis(conn: Connection) -> None:
    ...
    task_id, sel_run = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    _snap_id, tss_id = seed_source(conn, task_id, meta={"title": "Title only doc"})
    _seed_selection(
        conn, task_id, sel_run, scope_id,
        [{"tss_id": str(tss_id), "text_basis": "abstract_only"}],
    )
...
def test_determinism_across_identical_tasks(conn: Connection) -> None:
    """Two identical fixtures in separate tasks produce identical summary columns."""
    task_a, run_a = seed_task_and_run(conn)
    scope_a = seed_scope(conn, task_a)
    _seed_determinism_fixture(conn, task_a, run_a, scope_a)
    summary_a, _ = _run(conn, task_a, scope_a, run_a)

    task_b, run_b = seed_task_and_run(conn)
    scope_b = seed_scope(conn, task_b)
    _seed_determinism_fixture(conn, task_b, run_b, scope_b)
    summary_b, _ = _run(conn, task_b, scope_b, run_b)
    ...
    def _doc_sequence(summary: dict[str, Any]) -> list[dict[str, Any]]:
        return [
            {k: v for k, v in doc.items() if k not in {"tss_id", "extraction_record_id"}}
...
def test_parallel_vs_serial_same_write_order(
    ...
    try:
        task_default, run_default = seed_task_and_run(conn)
        scope_default = seed_scope(conn, task_default)
        _seed_determinism_fixture(conn, task_default, run_default, scope_default)
        captured.clear()
        summary_default, _ = _run(conn, task_default, scope_default, run_default)
        default_order = list(captured)

        monkeypatch.setattr(extract, "MAX_CONCURRENT_EXTRACT", 1)
        task_serial, run_serial = seed_task_and_run(conn)
        scope_serial = seed_scope(conn, task_serial)
        _seed_determinism_fixture(conn, task_serial, run_serial, scope_serial)
        captured.clear()
        summary_serial, _ = _run(conn, task_serial, scope_serial, run_serial)
...
def test_delete_task_data_removes_task011_rows(conn: Connection) -> None:
    """delete_task_data removes all three task-011 tables' rows without an IntegrityError."""
    task_id, sel_run = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Delete doc",
...
def test_summary_payload_shape(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Shape doc",
    ...
    for doc in summary["docs"]:
        assert set(doc.keys()) == {"tss_id", "basis", "profiles"}
