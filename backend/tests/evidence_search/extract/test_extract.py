...
from policy_atlas.evidence_search.extract.icf_prompt import ICF_PROMPT_VERSION
from policy_atlas.evidence_search.extract.icf_records import SCHEMA_VERSION as ICF_SCHEMA_VERSION
...
from policy_atlas.evidence_search.synthesis.synthesis_tools import make_findings_reader
...
def _seed_full_text_doc(
    ...
    *,
    ...
    """Seed a full-text doc (envelope + full-text snapshot + one chunk). Returns (tss, chunk).
...
def _seed_abstract_doc(
    ...
    *,
    ...
    """Seed an abstract-only doc (envelope snapshot, no chunks). Returns tss id."""
    envelope_snap = uuid.uuid4()
    tss_id = uuid.uuid4()
...
def _seed_selection(
    ...
    conn.execute(selection_result.insert().values(
        selection_result_id=uuid.uuid4(), task_id=task_id, evidence_scope_id=scope_id,
...
def _finding_count(conn: Connection, task_id: uuid.UUID) -> int:
...
def test_fresh_full_text_extraction(conn: Connection) -> None:
    ...
    tss_id, chunk_id = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Tutoring RCT", chunk_content=content,
        chunk_id=cid,
        stub_iof=[_record(
    ...
    assert [d["tss_id"] for d in summary["docs"]] == [str(tss_id)]
    ...
    rollup = conn.execute(
        select(extraction_result).where(extraction_result.c.task_id == task_id)
    ).one()
...
def test_both_profiles_write_separate_records_and_rollup_shape(conn: Connection) -> None:
    ...
    summary, run_id = _run(
        conn, task_id, scope_id, sel_run, profiles=(IOF_PROFILE_ID, ICF_PROFILE_ID)
    )
...
def test_profile_not_selected_differs_from_selected_zero_findings(conn: Connection) -> None:
    ...
    iof_only, _ = _run(conn, task_id, scope_id, sel_run)
...
def test_memo_reuse(conn: Connection) -> None:
    ...
    cid = uuid.uuid4()
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Home visiting", chunk_content=content,
        chunk_id=cid,
        stub_iof=[_record(
    ...
    findings_after_first = _finding_count(conn, task_id)
    ...
    assert _finding_count(conn, task_id) == findings_after_first
...
def test_memo_reuse_refresh_absent_is_byte_identical_to_as_built(conn: Connection) -> None:
    ...
    second, _ = _run(conn, task_id, scope_id, sel_run, refresh=None)
...
def test_refresh_abstract_only_bypasses_only_that_class(conn: Connection) -> None:
    ...
    abstract_tss_id = _seed_abstract_doc(
        conn, task_id, title="Mentoring", abstract=abstract,
        stub_iof=[_record(
    ...
    cid = uuid.uuid4()
    full_text_tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Home visiting",
    ...
    _seed_selection(conn, task_id, sel_run, scope_id, [
        {"tss_id": str(abstract_tss_id), "text_basis": "abstract_only"},
        {"tss_id": str(full_text_tss_id), "text_basis": "full_text"},
    ])

    first, _ = _run(conn, task_id, scope_id, sel_run)
    by_tss_first = {d["tss_id"]: d for d in profile_docs(first)}
    abstract_record_id_first = by_tss_first[str(abstract_tss_id)]["extraction_record_id"]
    full_text_record_id_first = by_tss_first[str(full_text_tss_id)]["extraction_record_id"]

    second, _ = _run(conn, task_id, scope_id, sel_run, refresh="abstract_only")

    assert second["provenance"]["refresh"] == "abstract_only"
    by_tss_second = {d["tss_id"]: d for d in profile_docs(second)}
    # The abstract_only doc is re-extracted fresh (bypassed the memo hit)...
    assert by_tss_second[str(abstract_tss_id)]["reused"] is False
    assert (
        by_tss_second[str(abstract_tss_id)]["extraction_record_id"]
        != abstract_record_id_first
    )
    # ...while the full_text doc's memo hit is untouched (not in the refreshed class).
    assert by_tss_second[str(full_text_tss_id)]["reused"] is True
    assert (
        by_tss_second[str(full_text_tss_id)]["extraction_record_id"]
...
def test_refresh_all_bypasses_every_memo_hit(conn: Connection) -> None:
    ...
    second, _ = _run(conn, task_id, scope_id, sel_run, refresh="all")
...
def test_refresh_failed_class_leaves_successful_memo_hits_untouched(
    ...
    ok_chunk = uuid.uuid4()
    ok_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Good doc",
    ...
    fail_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Bad doc",
        chunk_content="Some content here.", stub_failed=True,
    )
    _seed_selection(conn, task_id, sel_run, scope_id, [
        {"tss_id": str(ok_tss), "text_basis": "full_text"},
        {"tss_id": str(fail_tss), "text_basis": "full_text"},
    ])

    first, _ = _run(conn, task_id, scope_id, sel_run)
    by_tss_first = {d["tss_id"]: d for d in profile_docs(first)}
    assert by_tss_first[str(fail_tss)]["status"] == "extraction_failed"
    ok_record_id_first = by_tss_first[str(ok_tss)]["extraction_record_id"]

    second, _ = _run(conn, task_id, scope_id, sel_run, refresh="failed")
    ...
    assert by_tss_second[str(fail_tss)]["reused"] is False
    assert by_tss_second[str(fail_tss)]["status"] == "extraction_failed"
    # The sibling successful memo hit is untouched — refresh='failed' never
    # bypasses a successful record.
    assert by_tss_second[str(ok_tss)]["reused"] is True
    assert by_tss_second[str(ok_tss)]["extraction_record_id"] == ok_record_id_first
...
def test_fingerprint_change_extracts_fresh_alongside(
    ...
    assert _finding_count(conn, task_id) == 2 * findings_after_first
    ...
    third, _ = _run(conn, task_id, scope_id, sel_run)
...
def test_window_failure_does_not_fail_component(conn: Connection) -> None:
    ...
    by_tss = {d["tss_id"]: d for d in profile_docs(summary)}
    assert by_tss[str(fail_tss)]["status"] == "extraction_failed"
    assert by_tss[str(fail_tss)]["error"] == "window_failed: RuntimeError"
    assert by_tss[str(ok_tss)]["status"] == "extracted"
...
def test_no_findings_and_memo(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, task_id)
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Silent doc",
...
def test_empty_selection(conn: Connection) -> None:
    ...
    summary, run_id = _run(conn, task_id, scope_id, sel_run)
...
def test_nul_bearing_model_output_is_scrubbed(conn: Connection) -> None:
    ...
    tss_id, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="NUL doc", chunk_content=content,
    )
    _seed_selection(conn, task_id, sel_run, scope_id,
                    [{"tss_id": str(tss_id), "text_basis": "full_text"}])
...
def test_evidence_type_provenance_recorded_on_attempted_call(conn: Connection) -> None:
    ...
    scope_id = seed_scope(conn, task_id)
    cid = uuid.uuid4()
    classified_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Classified doc",
        chunk_content="Coaching increased graduation rates in the trial.",
    ...
    unclassified_tss, _ = _seed_full_text_doc(
        conn, task_id, sel_run, scope_id, title="Unclassified doc",
        chunk_content="Home visiting reduced admissions in the trial.",
        evidence_type=None,
    )
    _seed_selection(conn, task_id, sel_run, scope_id, [
        {"tss_id": str(classified_tss), "text_basis": "full_text"},
        {"tss_id": str(unclassified_tss), "text_basis": "full_text"},
    ])
    ...
    rows = {
        row.task_source_snapshot_id: row.primary_evidence_type
    ...
    assert rows[classified_tss] == "RCTs and Quasi-Experimental Studies"
    assert rows[unclassified_tss] == iof_prompt.UNCLASSIFIED_EVIDENCE_TYPE
...
def test_evidence_type_provenance_null_on_pre_prompt_failure(conn: Connection) -> None:
    ...
    task_id, sel_run = seed_task_and_run(conn)
    scope_id = seed_scope(conn, task_id)
    tss_id = _seed_abstract_doc(conn, task_id, title="Empty abstract doc", abstract="")
    _seed_selection(conn, task_id, sel_run, scope_id,
                    [{"tss_id": str(tss_id), "text_basis": "abstract_only"}])
...
def test_pre_existing_icf_v1_row_reader_keeps_context_label_coverage_absent(
    ...
    scope_id = seed_scope(conn, task_id)
    snap_id, tss_id = seed_source(conn, task_id, meta={"title": "ICF v1 doc"})
    _seed_selection(
        conn,
        task_id,
        run_id,
...
def test_stub_icf_extract_writes_context_label_and_defaults_missing_to_null(
    ...
    summary, _ = _run(
        conn, task_id, scope_id, sel_run, profiles=(ICF_PROFILE_ID,)
    )
